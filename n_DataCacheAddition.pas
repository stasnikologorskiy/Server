...
//--------------------------------------------------------------------- связка 2
  ...
  public
    ...
    property NodeHasPLs    : boolean index ik8_6 read GetLinkBool write SetLinkBool; // признак наличия связей с Motul в узле или его ветке
    property Qty: Single read FQty write FQty;       // количество
    function GetWareCodes: Tai;                      // must Free, массив кодов товаров связки 2
  end;

//------------------------------- Модель автомобиля / мотоцикла /грузовика / оси
  TModelParams = Class  // параметры модели
  private
    function GetStrMP(const ik: T16InfoKinds): String; // получить строку для вывода (property ...Out)
  public
    ...
    pCCM       : Word;     // PC,CV - Тех. обьем куб.см., AX - Ширина колеи [мм]
    pKW        : Word;     // Мощность кВт
    pHP        : Word;     // Мощность ЛС
    pCylinders : Word;     // Количество цилиндров
    pValves    : Word;     // PC - Кол-во клапанов на камеру сгорания, CV - тоннаж * 100
    pBodyID    : Word;     // PC - Код, тип кузова          (TYPEDIR=1)
                           //   CV - Код, Конструкция      (TYPEDIR=20)
                           //   AX - Код, Балка моста      (TYPEDIR=24)
    pDriveID   : Word;     // PC - Код, тип привода         (TYPEDIR=2)
                           //   CV - Код, Конфигурация оси (TYPEDIR=21)
                           //   AX - Код, Тип оси          (TYPEDIR=22)
    pEngTypeID : Word;     // PC,CV - Код, тип двигателя    (TYPEDIR=3)
                           //   AX - Код, Исполнение оси   (TYPEDIR=23)
    pFuelID    : Word;     // PC - Код, тип топлива         (TYPEDIR=4)
                           //   AX - Код, Колесное крепление (TYPEDIR=25)
    pFuelSupID : Word;     // Код, система впрыска          (TYPEDIR=5)
    pBrakeID   : Word;     // PC,AX - Код, тип тормозной системы (TYPEDIR=6)
    pBrakeSysID: Word;     // Код, Тормозная система        (TYPEDIR=7)
    pCatalID   : Word;     // Код, Тип катализатора         (TYPEDIR=8)
    pTransID   : Word;     // PC - Код, Вид коробки передач (TYPEDIR=9)
                           //   AX - Код, Hub system       (TYPEDIR=30)
    cvHPaxLO   : String;   // CV - Мощность [лс](от-до),  AX - Нагрузка[кг](от-до)
    cvKWaxDI   : String;   // CV - Мощность [кВт](от-до), AX - Дистанция[мм](от-до)
    cvSecTypes : String;   // Второстепенный тип            (список)
    cvWheels   : String;   // Колесная база [поз.осей]/[мм] (список)
    cvIDaxBT   : String;   // CV - ID производителя,  AX - Тип модели (список)
    cvCabs     : String;   // Кабина (список)
    cvAxles    : String;   // Ось [поз.оси]/[тип] (список код поз.оси TYPEDIR=38/код модели оси)
    cvSUAxBR   : String;   // CV - Подвеска/амортизация (список кодов TYPEDIR=cvtSusp)
                           //    AX - Размеры тормоза   (список кодов TYPEDIR=axtBrSize)
    property cvTonnOut  : String  index ik16_1 read GetStrMP;  // строка для вывода
    property cvHPaxLOout: String  index ik16_2 read GetStrMP;  // строка для вывода
    property cvKWaxDIout: String  index ik16_3 read GetStrMP;  // строка для вывода
    property cvSecTypOut: String  index ik16_4 read GetStrMP;  // строка для вывода
    property cvIDaxBTout: String  index ik16_5 read GetStrMP;  // строка для вывода
    property cvCabsOut  : String  index ik16_6 read GetStrMP;  // строка для вывода
    property cvWheelsOut: String  index ik16_7 read GetStrMP;  // строка для вывода
    property cvSUAxBRout: String  index ik16_8 read GetStrMP;  // строка для вывода
    property cvAxlesOut : String  index ik16_9 read GetStrMP;  // строка для вывода
  end;
  ...
  public
    ...
    property ModelHasPLs   : Boolean index ik8_5 read GetDirBool write SetDirBool; // Признак наличия связок модели с прод.линейками
    ...
    property Params        : TModelParams read FParams;       // параметры модели систем 1,2,4,5
...
//-------------------------------------------------------------------- Двигатели
  ...
  public
    ...
    pDesign     : Word;    // Код, Исполнение двигателя   (KT 96) (TYPEDIR=13)
    pFuelType   : Word;    // Код, Вид топлива            (KT 88) (TYPEDIR=12)
    pFuelMixt   : Word;    // Код, Система впрыска        (KT 97) (TYPEDIR=5)
    pAspir      : Word;    // Код, Подача воздуха         (KT 99) (TYPEDIR=14)
    pType       : Word;    // Код, Вид двигателя          (KT 80) (TYPEDIR=3)
    pNorm       : Word;    // Код, Норма выхлопных газов  (KT 63) (TYPEDIR=15)
    pCylDesign  : Word;    // Код, Тип привода ГРМ        (KT 79) (TYPEDIR=16)
    pManag      : Word;    // Код, Привод распредвала     (KT 77) (TYPEDIR=17)
    pValCnt     : Word;    // Код, Привод клапана         (KT 78) (TYPEDIR=18)
    pCoolType   : Word;    // Код, Тип системы охлаждения (KT 76) (TYPEDIR=19)
    pIsAuto     : Boolean; // Двигатель легк.авто
    pIsCV       : Boolean; // Двигатель груз.авто
  ...
  public
    ...
    property EngByAuto  : boolean index ik8_5 read GetDirBool write SetDirBool; // двигатель для легковых авто
    property EngByCV    : boolean index ik8_6 read GetDirBool write SetDirBool; // двигатель для грузовиков
  end;
  ...
  public
    ...
    function EditEngine(pID: Integer; pTDcode, pMFau, pSys, pUserID: Integer; // изменить двигатель
                       pMark: String; eps: TEngParams): String;
...
//-------------------------------------------------------- Производитель техники
  ...
  private // FID, FName, State - Код, Наименование производителя, Статус, FSubCode - Код TecDoc
    ...
    function GetBoolMf(ik: T16InfoKinds): boolean;        // получить признак
    procedure SetBoolMf(ik: T16InfoKinds; Value: boolean); // записать признак
  public
    ...
    function CheckHasModelLines(pTypeSys: Integer = 0): Boolean;   // Проверка имеютеся ли у производителя модельные ряды для системы учета
    ...
    function GetMfMLineIDByTDcode(pmlTDnr: Integer; pSys: integer=constIsAuto): Integer; // получить код модельного ряда авто по коду TecDoc
    property IsMF         : Boolean index ik16_1  read GetBoolMf  write SetBoolMf; // Признак производителя с моделями (F- только ОН)
    property IsMfAUTO     : Boolean index ik16_2  read GetBoolMf  write SetBoolMf; // Признак производителя авто
    property IsMfMOTO     : Boolean index ik16_3  read GetBoolMf  write SetBoolMf; // Признак производителя мото
    property IsMfTopA     : Boolean index ik16_4  read GetBoolMf  write SetBoolMf; // Признак топ-производителя авто
    property IsMfTopM     : Boolean index ik16_5  read GetBoolMf  write SetBoolMf; // Признак топ-производителя мото
    property IsMfVisA     : Boolean index ik16_6  read GetBoolMf  write SetBoolMf; // Признак видимости производителя авто
    property IsMfVisM     : Boolean index ik16_7  read GetBoolMf  write SetBoolMf; // Признак видимости производителя мото

    property IsMfCV       : Boolean index ik16_8  read GetBoolMf  write SetBoolMf; // Признак производителя груз.авто
    property IsMfAx       : Boolean index ik16_9  read GetBoolMf  write SetBoolMf; // Признак производителя осей
//    property IsMfVisCV    : Boolean index ik16_10 read GetBoolMf  write SetBoolMf; // Признак видимости производителя груз.авто
//    property IsMfVisAx    : Boolean index ik16_11 read GetBoolMf  write SetBoolMf; // Признак видимости производителя осей
//    property IsMfTopCV    : Boolean index ik16_12 read GetBoolMf  write SetBoolMf; // Признак топ-производителя груз.авто
//    property IsMfTopAx    : Boolean index ik16_13 read GetBoolMf  write SetBoolMf; // Признак топ-производителя осей
    property IsMfEng      : Boolean index ik16_14 read GetBoolMf  write SetBoolMf; // Признак производителя двигателей
//    property IsMfVisEng   : Boolean index ik16_15 read GetBoolMf  write SetBoolMf; // Признак видимости производителя двигателей
//    property IsMfTopEng   : Boolean index ik16_16 read GetBoolMf  write SetBoolMf; // Признак топ-производителя двигателей
...
//    property HasLinks: Boolean index ik8_4 read GetDirBool write SetDirBool; // признак наличия связок с моделями
...
//--------------------------------------------- TYPESINFOMODEL - списки по типам
  ...
  public
    ...
    function GetTypeName(pType: Integer): String;   // название типа списка
    function GetItemTypeName(pID: Integer): String; // название типа списка по ID элемента
    ...
    function FindInfoItemByValue(var pID: Integer; pType: Word; Value: String): Boolean; // найти код элемента списка по значению
...
//    FTreeNodes        : TTreeNodes;        // Дерево узлов
  ...
  public
    ...
    function AddNewOrigNumToCache(pID, pMfau: Integer; pNum: String; step: Integer=100): TOriginalNumInfo;  // добавление оригинального номера в кеш
...
//========================================== название типа списка по ID элемента
...
//======================================== найти код элемента списка по значению
...
begin
  ...
  try
    ...
    if not NodeGet(pParentID, pNodeParent) then // Узел родителя не найден
      raise Exception.Create(MessText(mtkNotFoundNode, IntToStr(pParentID)));

    if NodeGet(pID, pNodeAdd) then raise Exception.Create('Дубликат кода узла');
...
// проверка на наличие товаров у ноды родителя ???
    if Assigned(pNodeParent.Children) then
      if pNodeParent.Children.Find(pName, idx) then begin
        pNodeAdd:= TAutoTreeNode(pNodeParent.Children.Objects[idx]); // дубликат
  ...
  try
    ...
    if flUpdSysName then with NodesList do for idxParent2:= 0 to Count-1 do
      if Assigned(Objects[idxParent2]) then with TAutoTreeNode(Objects[idxParent2]) do
        if (ID<>pNodeID) and (AnsiUpperCase(NameSys)=pNameSys) then // Имеется узел с таким же системным именем, дубликат
...
begin
  ...
  for i:= 0 to Links.LinkCount-1 do begin    // идем по аналогам
    ...
    if analog.IsINFOgr or not analog.IsMarketWare then Continue; // инфо и без цен пропускаем
...
begin
  ...
  try
    ...
    if (High(FarOriginalNumInfo)<pID) then begin
      ...
      SetLength(FarOriginalNumInfo, pID+step);  // и инициируем элементы
          ...
          if CheckManufItem(fMFAUCODE, flNew, fMFAUNAME, fMFNR) then  // +100
            with FarManufacturers[fMFAUCODE] do begin
              ...
              if flNew then begin
                ...
                IsMFCV  := GetBoolGB(ORD_IBS, 'MFAUISMFCV');      // Признак производителя груз.авто
                IsMFAx  := GetBoolGB(ORD_IBS, 'MFAUISMFAX');      // Признак производителя осей
              end else begin
                ...
                fl:= GetBoolGB(ORD_IBS, 'MFAUISMFCV');      // Признак производителя груз.авто
                if (fl<>IsMFCV) then begin
                  IsMFCV:= fl;
                  fLists:= fLists or True;
                end;
                fl:= GetBoolGB(ORD_IBS, 'MFAUISMFAX');      // Признак производителя осей
...
{              if flNew then begin
//                IsMFVisCV := GetBoolGB(ORD_IBS, 'MFAUVISIBLECV');  // Признак видимости производителя груз.авто
//                IsMFTopCV := GetBoolGB(ORD_IBS, 'MFAUISTOPCV');    // Признак топ-производителя груз.авто
//                IsMFVisAx := GetBoolGB(ORD_IBS, 'MFAUVISIBLEAX');  // Признак видимости производителя осей
//                IsMFTopAx := GetBoolGB(ORD_IBS, 'MFAUISTOPAX');    // Признак топ-производителя осей
//                IsMfEng   := GetBoolGB(ORD_IBS, 'MFAUISMFeng');    // Признак производителя двигателей
//                IsMfVisEng:= GetBoolGB(ORD_IBS, 'MFAUVISIBLEeng'); // Признак видимости производителя двигателей
//                IsMfTopEng:= GetBoolGB(ORD_IBS, 'MFAUISTOPeng');   // Признак топ-производителя двигателей
              end else begin
                fl:= GetBoolGB(ORD_IBS, 'MFAUVISIBLECV');   // Признак видимости производителя груз.авто
                if (fl<>IsMFVisCV) then begin
                  IsMFVisCV:= fl;
                  fLists:= fLists or True;
                end;
                fl:= GetBoolGB(ORD_IBS, 'MFAUISTOPCV');     // Признак топ-производителя груз.авто
                if (fl<>IsMFTopCV) then begin
                  IsMFTopCV:= fl;
                  fLists:= fLists or True;
                end;
                fl:= GetBoolGB(ORD_IBS, 'MFAUVISIBLEAX');   // Признак видимости производителя осей
                if (fl<>IsMFVisAx) then begin
                  IsMFVisAx:= fl;
                  fLists:= fLists or True;
                end;
                fl:= GetBoolGB(ORD_IBS, 'MFAUISTOPAX');      // Признак топ-производителя осей
                if (fl<>IsMFTopAx) then begin
                  IsMFTopAx:= fl;
                  fLists:= fLists or True;
                end;
                fl:= GetBoolGB(ORD_IBS, 'MFAUISMFeng');      // Признак производителя двигателей
                if (fl<>IsMfEng) then begin
                  IsMfEng:= fl;
                  fLists:= fLists or True;
                end;
                fl:= GetBoolGB(ORD_IBS, 'MFAUVISIBLEeng');   // Признак видимости производителя двигателей
                if (fl<>IsMfVisEng) then begin
                  IsMfVisEng:= fl;
                  fLists:= fLists or True;
                end;
                fl:= GetBoolGB(ORD_IBS, 'MFAUISTOPeng');      // Признак топ-производителя двигателей
...
var ORD_IBS: TIBSQL;
    ORD_IBD: TIBDatabase;
    TimeProc: TDateTime;
    pName, sCV, sAx: String;      // Наименование Модели
...
fnIfStr(flmyDebug, ' "" KWfrto, "" HPfrto, "" sect, "" Wheel, "" ids, "" cabs, "" Susp, "" cvAxles', // для ускорения загрузки
    ...
    while not ORD_IBS.Eof do begin
      ...
      while not ORD_IBS.Eof and (ModelLineID=ORD_IBS.FieldByName('DMOSDRMLCODE').AsInteger) do begin
        ...
        flHasWares     := (ORD_IBS.FieldByName('hasWares').AsInteger=1);       // признак наличия связок с товарами
        flHasPLs       := (ORD_IBS.FieldByName('HasPLs').AsInteger=1);         // признак наличия связок с прод.линейками
        mps.cvKWaxDI   := ORD_IBS.FieldByName('KWfrto').AsString;              // CV - Мощность [кВт], AX - Дистанция [мм]
        mps.cvHPaxLO   := ORD_IBS.FieldByName('HPfrto').AsString;              // CV - Мощность [лс] , AX - Нагрузка на ось [кг]
        mps.cvSecTypes := ORD_IBS.FieldByName('sect').AsString;                // CV - Второстепенный тип
        mps.cvWheels   := ORD_IBS.FieldByName('Wheel').AsString;               // CV - Колесная база [поз.осей]/[мм]
        mps.cvIDaxBT   := ORD_IBS.FieldByName('ids').AsString;                 // CV - ID производителя,  AX - Тип модели
        mps.cvCabs     := ORD_IBS.FieldByName('cabs').AsString;                // CV - Кабина
        mps.cvSUAxBR   := ORD_IBS.FieldByName('Susp').AsString;                // CV - Подвеска/амортизация (список кодов TYPEDIR=cvtSusp)
                                                                               //   AX - Размеры тормоза    (список кодов TYPEDIR=axtBrSize)
        mps.cvAxles    := ORD_IBS.FieldByName('cvAxles').AsString;             // CV - Ось [поз.оси]/[тип]  (список: код TYPEDIR=cvtAxPos/код оси)
        ...
        try
          ...
          if fadd then begin
            ...
            if (model.FSubCode<>pModelTDcode) then model.FSubCode:= pModelTDcode; // Код TecDoc
          end;
          model.SetModelParams(mps, fadd);
          model.ModelHasWares:= flHasWares;         // признак наличия товаров модели
          model.ModelHasPLs:= flHasPLs;             // признак наличия прод.линеек
          flmlhas:= flmlhas or model.ModelHasWares; // признак наличия товаров мод.ряда
          model.State:= True;
        finally
          CS_Models.Leave;
        end;
...
//        ' -  марк. '+FarModels[OrdNum].MarksCommaText, fLogDebug, false);    // отладка
  except
    on E: Exception do prMessageLOGS(nmProc+': '+E.Message, fLogCache);
  end;
  prFreeIBSQL(ORD_IBS);
...
var ORD_IBS: TIBSQL;
  ...
  begin
    ...
    if not assigned(lstLinks[index]) then  // если линка на ноду нет в списке - добавляем
      ...
      link2:= lstLinks[index];             // если признаки уже есть - выходим
    ...
    AddParentLink(pNode.ParentID, hasw, hasM); // добавляем родителей / проставляем признак у родителей
  end;
  //----------------------------------
begin
  Result:= 0;
      ...
      while not ORD_IBS.Eof do begin
        ...
        while not ORD_IBS.Eof and (pTypeSys=ORD_IBS.FieldByName('DTSY').AsInteger) do begin
          ...
          if (lstLinks.Count<1) then with lstLinks do begin
            Capacity:= iCount;  // заполняем заготовку линков по кол-ву узлов системы
  ...
  //----------------------------------- обрабатываем 1 модель
          while not ORD_IBS.Eof and (pTypeSys=ORD_IBS.FieldByName('DTSY').AsInteger)
            ...
            if (index>-1) then begin // признак наличия товаров проставляем здесь, при заполнении связок 3
              ...
              HasMotul:= (ORD_IBS.FieldByName('hasPLs').AsInteger=1); // признак наличия связи с Motul
              ...
              AddParentLink(pNode.ParentID, haswares, HasMotul); // добавляем родителей
//              pNode.HasLinks:= True; // признак наличия 2-х связок
            end else begin
      ...
      resDeleted, resWrong:
        if not fex then begin
          ...
          if not Cache.CheckLinkAllowWrong(LinkSrc) then begin
            ...
            srcID:= linkSrc;
            OpCode:= resDeleted; // связки не TecDoc удаляем
          end;
...
//-------------------------------------------------------------------- проверяем
    if not (OpCode in [resAdded, resDeleted, resWrong, resNotWrong]) then
      raise EBOBError.Create(MessText(mtkNotValidParam)+' операции');
    if (userID<1) then raise Exception.Create(MessText(mtkNotValidParam)+' юзера');
    if not WareExist(WareID) then raise EBOBError.Create(MessText(mtkNotValidParam)+' товара');
    if (OpCode=resAdded) and (srcID<1) then
      raise EBOBError.Create(MessText(mtkNotValidParam)+' источника');

    Ware:= GetWare(WareID);
    if ware.IsArchive then raise EBOBError.Create(MessText(mtkNotValidParam)+' товара');
    ...
    if (OpCode<>resAdded) and (onID<1) then
      raise Exception.Create(MessText(mtkNotValidParam)+' ориг.номера');

    pONumLinks:= Ware.ONumLinks;
    if (OpCode in [resAdded, resNotWrong]) // проверяем добавление
      and pONumLinks.LinkListItemExists(onID, lkLnkByID) then begin
      ResCode:= resDoNothing;
      s:= MessText(mtkWareOrNumLink);
      if (OpCode=resAdded) then s:= s+' уже есть' else s:= s+' не отмечена, как ошибочная';
      raise EBOBError.Create(s);
    end;
//--------------------------------------------------- отрабатываем запись в базу
    ORD_IBD:= cntsOrd.GetFreeCnt;
...
//---------------------------------------------------------- отрабатываем кеш ОН
    ...
    if (ResCode in [resAdded, resNotWrong]) then begin //--------------- добавляем
      if not Assigned(OrigNum) then begin // если новый ОН - создаем
        OrigNum:= AddNewOrigNumToCache(onID, pMfID, pONum);
        if Manufacturers.ManufExists(pMfID) then try // отрабатываем список ОН производителя
    ...
    end else if (ResCode in [resDeleted, resWrong]) then begin //------- удаляем
      ...
      if flDelON and Manufacturers.ManufExists(pMfID) then try // отрабатываем список ОН производителя
        Manufacturers.CS_Mfaus.Enter;
        with Manufacturers[pMfID].ManufSearchONlist do // удаляем ОН
...
{       'select ORNWMFAUCODE, ORNWCODE, ORLKCODEWARE,'+
      ...
      while not OrdIBS.EOF do begin
        ...
        flMfLst:= ManufAutoExist(mf); // признак заполнения списка ОН
        ...
        while not OrdIBS.EOF and (mf=OrdIBS.FieldByName('ORNWMFAUCODE').AsInteger) do begin
                ...
                with Ware.ONumLinks do begin
                  if fFill or not LinkListItemExists(OrigNumID, lkLnkByID) then begin
                    ...
                    link.State:= True;
                                       ...
                                       // если кол-во ссылок меньше критического
          ...
          // Отправка сообщения о превышении критич.кол-ва ссылок на ориг.номер
          if (i>MaxLinksWithOE) then try
            ...
            body.Add(' в кеш записано '+IntToStr(MaxLinksWithOE));
            s:= n_SysMailSend(fnGetSysAdresVlad(caeOnlyWorkDay),
              'Уведомление об ошибке заполнения кеша', Body, nil,  '', '', True);
...
// SRCLDELKIND - режим удаления связок: 0 - удалять записи (ручной ввод, Excel и т.п.), 1 - помечать как неверные (из TecDoc и т.п.)
      ibs.ExecQuery;
      while not ibs.Eof do begin
...
//=========================================== Получить список аналогов от связки
//============================= оригинальных номеров с учетом производителя авто
...
begin
  Result:= -1;
  if not Assigned(self) or (TypeSys=2) then Exit; // по коду TecDoc ищем только авто !!!
...
//================ Проверка имеются ли у производителя модельные ряды по системе
...
begin
  ...
  try
    if not Assigned(self) then raise Exception.Create(MessText(mtkErrProcess));
    if not Cache.FDCA.ModelLines.ModelLineExists(pModelLineID) then
      raise Exception.Create(MessText(mtkNotFoundModLine, IntToStr(pModelLineID))); // Модельный ряд с таким кодом отсутствует
...
//=== сортировка TStringList производителей легк.авто - видимость + наименование
...
{//== сортировка TStringList производителей двигателей - видимость + наименование
...
//=== сортировка TStringList производителей груз.авто - видимость + наименование
function CVManufSortCompare(List: TStringList; Index1, Index2: Integer): Integer;
begin
  Result:= ManufSort(List, Index1, Index2, constIsCV);
end;
//======== сортировка TStringList производителей осей - видимость + наименование
...
//======== Возвращает указатель на производителя с кодом pIndex или на 0 элемент
  ...
  if not Assigned(self) then Exit else case ik of // Наименование модели для сортировки
    ...
    ik8_5: begin                                     // Название модели для вывода в Web
           ...
           if Assigned(Params) then with Params do begin
             ...
             case TypeSys of
               constIsAuto: Result:= Result+' | '+IntToStr(pHP)+'лс | '+MarksCommaText;
               constIsCV  : Result:= Result+' | '+cvHPaxLOout+'лс | '+MarksCommaText;
               constIsAx  : Result:= Result+' | '+cvHPaxLOout+'кг | '+
...
//================================= получить строку для вывода (property ...Out)
function TModelParams.GetStrMP(const ik: T16InfoKinds): String;
var i, j, ipos: integer;
    s, ss: String;
begin
  Result:= '';
  if not Assigned(self) then Exit else case ik of   // строка для вывода
    ...
    ik16_5: Result:= StringReplace(cvIDaxBT, ',', '<br>&nbsp', [rfReplaceAll]);  // списком
    ik16_6: Result:= StringReplace(cvCabs, ',', '<br>&nbsp', [rfReplaceAll]);    // списком
    ik16_7: begin
              Result:= StringReplace(cvWheels, ',', '<br>&nbsp', [rfReplaceAll]); // списком
    ...
    ik16_8: with fnSplit(',', cvSUAxBR) do try
              for i:= 0 to Count-1 do begin
                ...
                if (s='') then Continue;
                Result:= Result+fnIfStr(Result='', '', '<br>&nbsp')+ // списком
                                StringReplace(s, '/', ' / ', [rfReplaceAll]);
              end; // for
            finally Free; end; // ik16_8
    ik16_9: with fnSplit(',', cvAxles) do try // Ось [поз.оси]/[тип] (список код поз.оси TYPEDIR=38/код модели оси)
              for i:= 0 to Count-1 do begin
                s:= '';
                ss:= '';
                ipos:= pos('/', Strings[i]);
                ...
                Result:= Result+fnIfStr(Result='', '', '<br>&nbsp')+s+' / '+ss; // списком
              end; // for
...
//  ORD_IBD:= nil;
  ...
  flCV:= (FParams.cvKWaxDI<>mps.cvKWaxDI) or (FParams.cvHPaxLO<>mps.cvHPaxLO); // грузовики
  sSql:= '';
    ...
    while not ORD_IBS.Eof do begin
      ...
      if (index>-1) then begin                  // признак наличия товаров
        ...
        lstLinks[index]:= TSecondLink.Create(ORD_IBS.FieldByName('Rsrc').AsInteger,
          ORD_IBS.FieldByName('RCount').AsFloat, pNode, True, haswares, hasFilters, HasPLs); // заносим связку узла
        AddParentLink(pNode.ParentID, haswares, HasPLs); // добавляем родителей
      end else begin
        prMessageLOGS(nmProc+': not node ID= '+IntToStr(tnID), fLogCache+'_test', false);
        Inc(err2);
      end; // if index>-1
...
//---------------------------------------------------------------- DIRMODELS_add
      ...
      if (lst.Count>0) then begin
        for ii:= 0 to lst.Count-1 do begin
          ls.Add(lst[ii]);
          if (ls.Count>100) then begin // запись порции строк
        ...
        if (ls.Count>0) then begin // запись последней порции строк
          ls.Insert(0, 'execute block as begin');
          ls.Add(' end');
          ORD_IBS.SQL.Clear;
          ORD_IBS.SQL.AddStrings(ls);
...
//------------------------------------------------------------------------------
      ...
      if Assigned(eps) then with eps do begin
        ...
        sVal:= sVal+', '+fnIfStr(yfrom>0, IntToStr(yfrom),     'null')+ // Год выпуска от
          ', '+fnIfStr(yto>0,          IntToStr(yto),          'null')+ // Год выпуска до
          ', '+fnIfStr(pCCtecFrom>0,   IntToStr(pCCtecFrom),   'null')+ // Тех.обьем в куб.см. от
          ', '+fnIfStr(pCCtecTo>0,     IntToStr(pCCtecTo),     'null')+ // Тех.обьем в куб.см. до
          ', '+fnIfStr(pHPfrom>0,      IntToStr(pHPfrom),      'null')+ // Мощность ЛС от
          ', '+fnIfStr(pHPto>0,        IntToStr(pHPto),        'null')+ // Мощность ЛС до
          ', '+fnIfStr(pRPMKWFrom>0,   IntToStr(pRPMKWFrom),   'null')+ // при [об/мин] от
          ', '+fnIfStr(pRPMKWTo>0,     IntToStr(pRPMKWTo),     'null')+ // при [об/мин] до
          ', '+fnIfStr(pKWFrom>0,      IntToStr(pKWFrom),      'null')+ // Мощность кВт от
          ', '+fnIfStr(pKWTo>0,        IntToStr(pKWTo),        'null')+ // Мощность кВт до
          ', '+fnIfStr(pCompFrom>0,    IntToStr(pCompFrom),    'null')+ // Компрессор * 100 от
          ', '+fnIfStr(pCompTo>0,      IntToStr(pCompTo),      'null')+ // Компрессор * 100 до
          ', '+fnIfStr(pRPMtorqFrom>0, IntToStr(pRPMtorqFrom), 'null')+ // Момент вращения (Nm) при [об/мин] от
          ', '+fnIfStr(pRPMtorqTo>0,   IntToStr(pRPMtorqTo),   'null')+ // Момент вращения (Nm) при [об/мин] до
          ', '+fnIfStr(pStroke>0,      IntToStr(pStroke),      'null')+ // Ход поршня * 1000
          ', '+fnIfStr(pBore>0,        IntToStr(pBore),        'null')+ // Расточка * 1000
          ', '+fnIfStr(pVal>0,         IntToStr(pVal),         'null')+ // Количество клапанов
          ', '+fnIfStr(pCyl>0,         IntToStr(pCyl),         'null')+ // Количество цилиндров
          ', '+fnIfStr(pCrank>0,       IntToStr(pCrank),       'null')+ // Кол-во подшипников коленвала
          ', '+fnIfStr(pDesign>0,      IntToStr(pDesign),      'null')+ // Код, Исполнение двигателя   (KT 96) (TYPEDIR=13)
          ', '+fnIfStr(pFuelType>0,    IntToStr(pFuelType),    'null')+ // Код, Вид топлива            (KT 88) (TYPEDIR=12)
          ', '+fnIfStr(pFuelMixt>0,    IntToStr(pFuelMixt),    'null')+ // Код, Система впрыска        (KT 97) (TYPEDIR=5)
          ', '+fnIfStr(pAspir>0,       IntToStr(pAspir),       'null')+ // Код, Подача воздуха         (KT 99) (TYPEDIR=14)
          ', '+fnIfStr(pType>0,        IntToStr(pType),        'null')+ // Код, Вид двигателя          (KT 80) (TYPEDIR=3)
          ', '+fnIfStr(pNorm>0,        IntToStr(pNorm),        'null')+ // Код, Норма выхлопных газов  (KT 63) (TYPEDIR=15)
          ', '+fnIfStr(pCylDesign>0,   IntToStr(pCylDesign),   'null')+ // Код, Тип привода ГРМ        (KT 79) (TYPEDIR=16)
          ', '+fnIfStr(pManag>0,       IntToStr(pManag),       'null')+ // Код, Привод распредвала     (KT 77) (TYPEDIR=17)
          ', '+fnIfStr(pValCnt>0,      IntToStr(pValCnt),      'null')+ // Код, Привод клапана         (KT 78) (TYPEDIR=18)
          ', '+fnIfStr(pCoolType>0,    IntToStr(pCoolType),    'null')+ // Код, Тип системы охлаждения (KT 76) (TYPEDIR=19)
          ', '+fnIfStr(pSalesDesc<>'', ':SALESDESC',           'null'); // Обозначение продаж
      end; // if Assigned(eps)
