...
type
  ...
  TEmplInfo = record
    ...
    ShowImportPage: boolean; // Есть ли разрешенные типы импорта
    SysOptions: boolean; // Есть ли разрешение на управление константами
    ShowNewAttr: boolean; // Показатель показывать ли слово New  в атрибутах
    ...
    NeedTinyMCEAction: boolean; // true, если нужен редактор для Акций
    PageName: string;
  ...
  TUserInfo = record
    ...
    ballsName: string; // Имя для бонусов
    contBonusOrd: integer; // контракт юнит заказа формирующегося
    contBonusReserv: Double; // контракт юнит заказа формирующегося
    qvPriceCol: integer; // количество колонок  для цен
    // ------------------Блок переменных для скидок
    DirectParams: integer; // Кол-во направлений
    DirectName: String; // название направления
    LevelCount: integer; // Кол-во уровней
    FirmModel_Rating: integer; // Текущий уровень
    FirmModel_Sales: integer; // Показатель оборота текущего уровня
    NextModel_Rating: integer;
    // Уровень, до которого к/а не дотягивает по обороту текущего месяца
    NextModel_Sales: integer;
    // Показатель оборота уровня, до которого к/а не дотягивает
    ProcToNext: integer;
    // Процент участка шкалы, которого не хватает до уровня, до которого к/а не дотягивает
    FirmSales: integer; // Показатель текущего оборота к/а
    ...
    flRedesign: boolean ; // флаг для редизайна
    flUber: boolean;
    flInitMedia: boolean;
...
type
  TTypeFile = (tfBMP, tfICON, tfCURSOR, tfWAVE, tfMIDI,
    // для проверки типов файлов для jpeg
...
function scaleImages(var jpg: TJpegImage; Width: integer; Height: integer)
  : string; // функция для маштабирования картинок слайдера
...
function scaleImagesMotul(var jpg: TJpegImage; Width: integer; Height: integer)  : string; // функция для маштабирования картинок слайдера
...
begin
  ...
  Result := Result +
    '<button onclick="$(\''#btnSaveCriteria\'').val(\''Добавить критерий\''); $(\''#btnSaveCriteria\'').attr(\''rowcode\'', \''\''); $(\''#critname\'').val(\''\''); '
  // +'$(\''#critvalue\'').val(\''\''); $.fancybox.open($(\''#addcoudiv\''), {\''modal\'' : true, \''padding\'': 10});" id=uiAddCriteria>Добавить критерий</button>';
    + '$(\''#critvalue\'').val(\''\''); $(\''#addcoudiv\'').dialog(\''open\'');" id=uiAddCriteria>Добавить критерий</button>';
  Result := Result +
    '<button onclick="saveportionNew();" id=uiSavePortion>Сохранить изменения</button>';
  Result := Result +
    '<button onclick="if ($(\''#uiSavePortion\'').button( \''option\'', \''disabled\'') || confirm(\''Вы действительно хотите закрыть окно?\'')) $(\''#jqueryuidiv\'').dialog(\''destroy\'');">Закрыть окно</button>';
  Result := Result + '</div>';
end;
...
// выводит текст в maindiv со всеми обвязками. необходимыми для формирования окна с закруглениями и тенями
function fnWriteSimpleText(s: string): string;
begin
  Result := '';
  Result := Result +
...
// выдает запись для стиля с указанными left, top, width, height
function fnGetLTWHStyle(l, t, w, h: integer): string;
begin
...
// готовит строку сообщения к выводу с помощью alert в JavaScript
function GetJSSafeString(s: string): string;
begin
  s := StringReplace(s, '"', '`', [rfReplaceAll]);
...
begin
  ...
  StreamOut.WriteStr(s + fnWriteSimpleText('Ошибка выполнения: ' +
...
begin
  ...
  // назначаю функции выкидывания в контролы содержимого
  ...
  s := s + '      $("#mainnodename").html("Не определен для неконечных узлов");'#13#10;
  s := s + '    } else if ($(this).attr("code")==$(this).attr("mainnode")) {'#13#10;
  s := s + '      $("#mainnodename").html("Этот узел - главный");'#13#10;
  s := s + '    } else { '#13#10;
