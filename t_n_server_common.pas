...
var
  VSMail: TVSMail;
  nmIniFileBOB, cNoReplayEmail, cFictiveEmail: string;
  ...
  flCSSnew, flDebug, flTest, flTestDocs, flClientStoragesView_2col, flMargins,
    flTmpRecodeCSS, flTmpRecodeORD, flBonusAttr, flNewSaveAcc, flTD24, flCVax,
    flShowAttrImage, flNewModeCGI, flDisableOut, flRedesign, flUber,
    flClientStoragesView_add, flContManagerBKEonly{, flTopRating}: boolean;
...
//------------------------------------------------------------ vc
  ...
  thControlPayThread: TThread;      //vv
  thControlSMSThread: TThread;      //vv
...
procedure prSaveWareRestsExists(Stream: TBoBMemoryStream; ForFirmID, ContID: Integer; wCodes: Tai); overload; // запись в Stream блока семафоров наличия товаров (вызов только в Web)
 function fnGetContMainStoreAndStoreCodes(FirmID, ContID: Integer; var StorageCodes: Tai): Integer; // получить коды складо контракта и код главного склада
...
//procedure prFormMediaList(ThreadData: TThreadData);                        // сформировать файлы медиа-блоков
procedure prGetMediaBloksSrvMng(Stream: TBoBMemoryStream; ThreadData: TThreadData); // файл медиа-блоков

//procedure TestFile;

implementation
uses n_IBCntsPool, n_DataCacheObjects, n_DataCacheInMemory,
...
ErrorPos:= '8-8-2';
        case Command of
          ...
          csGetReclamationList          : prGetReclamationList(Stream, ThreadData);         // список рекламаций
...
var GBIBD: TIBDatabase;
    GBIBS: TIBSQL;
    iType, mbLast, fsize, i, j: integer;
    s, mbPath, mbFile, sHint, sWEBLink, MBnum: String;
    arMBnums: Tas;
...
//  LocalStart:= now();
  ...
  try
    ...
    GBIBS.SQL.Text:=
      ...
      '    mr1.MdBlShowInterval ShowOrder, mr1.MdBlHint Hint, mr1.MdBlWEBLink WEBLink,'+
      '    mr1.MdBlImage Image, mr1.MdBlImageExtn ImageExt, 0 WareCode, mr1.mdblcode'+
      ...
      '    mr2.MdBlShowInterval ShowOrder, mr2.MdBlHint Hint, mr2.MdBlWEBLink WEBLink,'+
      '    null Image, null ImageExt, MdBlWrWareCode WareCode, mr2.mdblcode'+
      ...
      '  order by BlockNum, ShowOrder, InfoType, mdblcode';
    GBIBS.ExecQuery;
        ...
        if (iType<>cmbBibb) then begin // Картинка, Товар (не заглушка)
          ...
          with arMedia[mbLast][j] do begin
            ...
            if (iType=cmbWare) then begin // товар
              ...
              WareName:= ware.Name;
              if ware.IsPrize then InfoType:= cmbPriz;
    ...
    for i:= 0 to High(arMBnums) do begin
      ...
      for j:= 0 to High(arMedia[i]) do with arMedia[i][j] do begin
        s:= IntToStr(InfoType)+'|'+ // тип: 0 - картинка, 1 - обычный товар, 2 - заглушка, 3 - подарок
            ...
            IntToStr(WareCode)+'|'+ // код товара для InfoType in [1,3], рисунок подтягивается по обычной схеме и вставляется в блок слева
            WareName+'|';           // наименование товара для InfoType in [1,3] - выводить в свободной области блока
        Stream.WriteStr(s);
        Stream.WriteStr(ImageFname); // имя файла картинки в папке media для InfoType in [0,2]
        if (ImageFname='') then
          Stream.WriteInt(0)
        else begin
          fsize:= ms.Size;
...
{
//============================================== сформировать файлы медиа-блоков
procedure prFormMediaList(ThreadData: TThreadData);
const nmProc = 'prFormMediaList'; // имя процедуры/функции
var GBIBD: TIBDatabase;
    GBIBS: TIBSQL;
    iCount, iType, mbLast: integer;
    s, mbPath, mbFile, sHint, sWEBLink, nf, BaseUrl, PathTo: String;
    arMBnums: Tas;
    arMedia: array of array of Rmedia;
    mBibb: Rmedia;
    ware: TWareInfo;
...
//  LocalStart:= now();
  ...
  try
    ...
    PathTo:= fnTestDirEnd(BaseUrl)+'media';

    nf:= mbPath+'media.ini';
    fnTestFileCreate(nf); // проверяет существование Ini-файла, если нет - создает
    if not FileExists(nf) then raise EBOBError.Create('Ошибка формирования файла медиа-блоков');
    ...
    if Assigned(ThreadData) then iType:= ThreadData.ID else iType:= -1;

    GBIBD:= CntsGRB.GetFreeCnt();
    GBIBS:= fnCreateNewIBSQL(GBIBD, 'GBIBS_'+nmProc, iType, tpRead, True);
    GBIBS.SQL.Text:=
      'select * from (select num.andtname BlockNum, mr1.MdBlInformationType InfoType,'+
      '    mr1.MdBlShowInterval ShowOrder, mr1.MdBlHint Hint, mr1.MdBlWEBLink WEBLink,'+
      '    mr1.MdBlImage Image, mr1.MdBlImageExtn ImageExt, 0 WareCode, mr1.mdblcode'+
...
//----------------------------------------------------------------- 1 медиа-блок
      ...
      while not GBIBS.EOF and (arMBnums[mbLast]=GBIBS.FieldByName('BlockNum').AsString) do begin
        ...
        if (iType<>cmbWare) then begin  // не товар - формируем имя файла картинки
          mbFile:= 'm_pic_'+arMBnums[mbLast]+'_'+IntToStr(iCount+1)+
         // GBIBS.FieldByName('mdblcode').AsString+
          GBIBS.FieldByName('ImageExt').AsString;
          GBIBS.FieldByName('Image').SaveToFile(mbPath+mbFile); // Blob в файл
        end;

        if (iType<>cmbBibb) then begin // Картинка, Товар (не заглушка)
          if (iCount>High(arMedia[mbLast])) then SetLength(arMedia[mbLast], iCount+10);
          with arMedia[mbLast][iCount] do begin
            InfoType:= iType;
            Hint:= sHint;
            WEBLink:= sWEBLink;
            if (iType=cmbWare) then begin // товар
          ...
          Hint:= sHint;
          WEBLink:= sWEBLink;
          // если несколько заглушек, удаляем файл предыдущей
          if (ImageFname<>'') then DeleteFile(mbPath+ImageFname);
          ImageFname:= mbFile;
        end; // with mBibb
      ...
      if (Length(arMedia[mbLast])>iCount) then SetLength(arMedia[mbLast], iCount);
                                          // если список пустой и есть заглушка
      if (Length(arMedia[mbLast])<1) and (mBibb.ImageFname<>'') then begin
        SetLength(arMedia[mbLast], 1);
        with arMedia[mbLast][0] do begin
    ...
    pIniFile:= TINIFile.Create(nf);
    try
      for iType:= 0 to High(arMBnums) do
        for iCount:= 0 to High(arMedia[iType]) do with arMedia[iType][iCount] do begin
          s:= IntToStr(InfoType)+'|'+ImageFname+'|'+WEBLink+'|'+Hint+'|'+IntToStr(WareCode)+'|'+WareName;
          pIniFile.WriteString(arMBnums[iType], 'm'+IntToStr(iCount+1), s);
// InfoType: 0 - картинка, 1 - обычный товар, 2 - картинка-заглушка, 3 - товар-подарок
// ImageFile - имя файла картинки в папке media для InfoType in [0,2]
// WEBLink - ссылка, куда идти по щелчку на картинке, если пусто - для InfoType in [1,3] открываем окно просмотра товара
// Hint - текст всплывающей подсказки
// WareCode - код товара для InfoType in [1,3], рисунок подтягивается по обычной схеме и вставляется в блок слева
// WareName - наименование товара для InfoType in [1,3] - выводить в свободной области блока
        end; // for iCount:= 0 to High(arMedia[iType])
    finally
      prFree(pIniFile);
    end;
...
//  tt:= StrToDate('01.01.2017');
  ...
  if flNewSaveAcc then CheckClosingDocsByPeriod_new(tbegin, tend)
  else CheckClosingDocsByPeriod(tbegin, tend);
end;
...
begin
  ...
  ibsOrd:= nil;
  LocalStart:= now();
  SetLength(Accounts, 100);
  SetLength(Invoices, 100);
  SetLength(arOrders, 100);
...
// ???
    if not ibsGB.Transaction.InTransaction then ibsGB.Transaction.StartTransaction;
    ibsGB.SQL.Text:= 'select PInvCode, PInvNumber, PInvDate, PInvCrncCode, piavOrdCode,'+
      '  PInvSupplyDprtCode, PInvSumm, PINVPROCESSED, PINVCLIENTCOMMENT, PINVANNULKEY,'+
      '  INVCCODE, INVCNUMBER, INVCDATE, INVCSUMM, INVCCRNCCODE, INVCSUPPLYDPRTCODE'+
      '  from (select p1.piavAccCode, p1.piavOrdCode from payinvalter_vlad p1'+
      '    where p1.piavLastTime between :tbegin and :tend'+
      ...
      '    where iavLastTime between :tbegin and :tend) s'+
    ...
    while not ibsGB.Eof do begin
      if GetBoolGB(ibsGB, 'PINVANNULKEY') then begin // если счет аннулирован
        lstBlock.Add('delete from OrdersClosingDocs where OCDAccCode='+
          ibsGB.FieldByName('PInvCode').AsString+';'); // удаляем строки по счету
        ibsGB.Next;
        Inc(AccCount);
        Inc(DelCount);
        Continue;
      end;
      if (High(Accounts)<accInd) then begin
        SetLength(Accounts, accInd+100);
        SetLength(Invoices, accInd+100);
        SetLength(arOrders, accInd+100);
      end;
      arOrders[accInd]:= ibsGB.FieldByName('piavOrdCode').AsInteger;
      with Accounts[accInd] do begin // заносим данные счета в структуру
        ID          := ibsGB.FieldByName('PInvCode').AsInteger;
        Number      := ibsGB.FieldByName('PInvNumber').AsString;
        Data        := ibsGB.FieldByName('PInvDate').AsDateTime;
        Summa       := ibsGB.FieldByName('PInvSumm').AsFloat;
      ...
      with Invoices[accInd] do begin // заносим данные накладной в структуру
    ...
    if (Length(Accounts)>0) then begin //---------- если есть счета для проверки
      if not ibsOrd.Transaction.InTransaction then ibsOrd.Transaction.StartTransaction;
      for accInd:= 0 to High(Accounts) do begin
        jj:= 0;
        jd:= 0;
        ibsOrd.Close;
        ibsOrd.SQL.Text:= 'select d.*, iif(ordrcode is null, '+
          IntToStr(orstNoDefinition)+', ORDRSTATUS) STATUS'+
          ' from OrdersClosingDocs d'+
          ' left join ordersreestr on ordrcode=d.OCDOrderCode'+
          ' where d.OCDAccCode='+IntToStr(Accounts[accInd].ID);
//          fnIfStr(arOrders[accInd]>0, ' and d.OCDOrderCode='+IntToStr(arOrders[accInd]), '');
        ibsOrd.ExecQuery;                         // не нашли - добавляем строку
        if (ibsOrd.Eof and ibsOrd.Bof and (arOrders[accInd]>0)) then begin
          with Accounts[accInd] do begin
            s:= 'OCDOrderCode, OCDAccCode, OCDAccNumber, OCDAccDate, OCDAccCrnc,'+
                ' OCDAccDprt, OCDAccSumm, OCDAccPROCESSED, OCDAccCOMMENT';
            ss:= IntToStr(arOrders[accInd])+', '+IntToStr(ID)+', "'+Number+'", "'+
...
//            ID:= 0; // счет из Grossbee проверен - обнуляем код
      ...
      end; // accInd:= 0 to High(Accounts)
      ibsOrd.Close;
    end;  // if (Length(Accounts)>0)
...
//    Order.STORAGE:= Order.Contract.MainStoreStr;
    ...
    if (Order.currID<>Cache.BonusCrncCode) then
      ...
      else if (Order.accType=1) then Order.currID:= cUAHCurrency
      else Order.currID:= cDefCurrency;
...
const nmProc = 'fnOrderToGB'; // имя процедуры/функции
      ...
      cstErrLock  = -1;
      cstErrFatal = -9;
var i, TryCount, ErrCount, RecCount, j, STORAGEnew: integer;
    ErrPos, s, s1, ss2, ss3, ErrorStr, WareErrStr, accLine, wCutName, sQty, sDoc2,
      sResQty, sDprtID, sAccID, sDivis, sDoc1, sDoc4, sOrderID, sContID, sCurrID: String;
    Success, flSaveCont, flDontJoin, flProcessed: boolean;
  ...
  begin
    ...
    end else if (pos('Order of ware more rest', EMessage)>0)
      or (pos('NewLineCode', EMessage)>0) then ss3:= EMessage
    else if (pos('заблокирован', EMessage)>0) then begin
      tc.ID2:= cstErrFatal; // фатальная ошибка
      ErrorStr:= ErrorStr+#13#10'товар заблокирован';
      Result:= True;
      exit;
    end;
    if (ss3<>'') then
      ...
      tc.ID2:= cstErrFatal; // фатальная ошибка
      if (EMessage<>'') then ErrorStr:= ErrorStr+#13#10+EMessage;
      Result:= True;
      exit;
  ...
  begin
    ...
    if Success then begin
//      tc.ID2:= cstSaved;
      if (ResQty>0) and (Ord.DprtID>0) then // снимаем с остатка в кеше фактическое кол-во
  ...
  begin
    ...
    sQty:= StringReplace(FloatToStr(RoundTo(tc.Qty, -3)), ',', '.', [rfReplaceAll]);
...
begin
  LocalStart:= now();
  ErrorStr:= '';   // сообщение о глобальной ошибке
  WareErrStr:= ''; // сообщение об ошибках записи товаров
  WaresErrMess:= '';
...
ErrPos:= '0';
    ...
    if (Ord.currID<>Cache.BonusCrncCode) then begin
      ...
      else if (Ord.accType=1) then Ord.currID:= cUAHCurrency
      else Ord.currID:= cDefCurrency;
...
//------------------------------------------------- проверяем параметры отгрузки
    if flCheckShipParams then begin
      ...
      if (s1<>'') then ErrorStr:= ErrorStr+#13#10+s1;
    end;

    ord.accJoin.ID:= 0;
    ord.accSing.ID:= 0;
//////////////////////////////////////////////////////////// счет для добавления
    ibsGBt.ParamCheck:= False;
    if not flDontJoin then try // ищем счет для добавления (все товары в наличии)
      ...
      ibsGBt.SQL.Add('execute block returns (AddAccCode integer, AddAccNumber varchar(20),'+
                     ' Processed varchar(1), webcomm varchar(255), aDATE timestamp)');
      ibsGBt.SQL.Add('as declare variable xFlag integer;'+
                     ' declare variable xOrder double precision;');
      ibsGBt.SQL.Add(' declare variable xCount double precision;'+
                     ' declare variable xRest double precision;');
      ibsGBt.SQL.Add('begin AddAccCode=0; webcomm=""; ');
      ibsGBt.SQL.Add(' for select PInvCode, PINVWEBCOMMENT, PInvProcessed,'+
                     '  PInvDate, pinvnumber from PayInvoiceReestr');
      ibsGBt.SQL.Add(' left join SubContract on sbcndocmtype='+IntToStr(docAccount)+
                     ' and sbcndocmcode=pinvcode');
      if (Ord.deliv=cDelivTimeTable) then      // Доставка по расписанию
        ibsGBt.SQL.Add(' left join TRANSPORTTIMETABLESLINES on TRTBLNCODE=pinvtripcode');
      ibsGBt.SQL.Add(' where PInvDate="today" and PInvRecipientCode='+IntToStr(Ord.Firma.ID)+
                     ' and PINVCONTRACTCODE='+sContID+' and sbcnactive="T"'+
                     ' and PInvDontJoin="F" and (PInvProcessed="T"');             // активные
      ibsGBt.SQL.Add(' or (PINVWEBCOMMENT is not null and PINVWEBCOMMENT<>""'+
                     '  and (PINVCLIENTCOMMENT is null or'+  // спец.признак - счет в работе
                     '   not (PINVCLIENTCOMMENT containing "'+cSpecDelim+'"))))');
      if (Ord.deliv=cDelivReserve) then        // резерв - метод отгрузки или дата пустые
        ibsGBt.SQL.Add(' and (pinvshipmentmethodcode is null or pinvshipmentdate is null)');
      if (Ord.deliv in [cDelivSelfGet, cDelivTimeTable]) then begin // дата отгрузки
        ibsGBt.SQL.Add(' and pinvshipmentdate="'+FormatDateTime(cDateFormatY4, Ord.pDate)+'"');
        if (Ord.deliv=cDelivSelfGet) then                // Самовывоз
          ibsGBt.SQL.Add(' and pinvshipmentmethodcode='+IntToStr(Ord.smID)+ // метод отгрузки - Самовывоз
                         ' and pinvshipmenttimecode='+IntToStr(Ord.stID));  // время отгрузки
        if (Ord.deliv=cDelivTimeTable) then              // Доставка по расписанию
          ibsGBt.SQL.Add(' and PINVSUPPLIERFIRMDPRT='+IntToStr(Ord.DestID)+ // торг.точка
                         ' and TRTBLNDOCMCODE='+IntToStr(Ord.ttID));        // расписание
      end; // if (Ord.deliv in [cDelivSelfGet, cDelivTimeTable])
      ibsGBt.SQL.Add(' into :AddAccCode, :webcomm, :Processed, :aDATE, :AddAccNumber do begin xFlag=1;');
      ibsGBt.SQL.Add('  for select PInvLnOrder, PInvLnCount, g.Rmarket from PayInvoiceLines');
      ibsGBt.SQL.Add('   left join GetWareRestsCSS_Vlad(PInvLnWareCode, '+sDprtID+') g on 1=1');
      ibsGBt.SQL.Add('   where PInvLnDocmCode=:AddAccCode into :xOrder, :xCount, :xRest do begin');
      ibsGBt.SQL.Add('   if (not (xOrder>0 and xCount>0 and xRest>=0)) then xFlag=0; end'); // проверка наличия товара // ???
      ibsGBt.SQL.Add('  if (xFlag>0) then begin suspend; exit; end end end'); // нашли - выходим
ErrPos:= '2';
      ibsGBt.ExecQuery;
      ...
      on E: Exception do begin
        ...
        ErrorStr:= ErrorStr+#13#10+e.Message;
      end;
    end;
    ibsGBt.Close;
...
//================================================ нашли счет - добавляем товары
      ...
      ibsGB.SQL.Add('  if (xOrder>xRest) then begin'); //--- если в наличии меньше заказа
                                                       //--- снимаем заказ по кратности
      ibsGB.SQL.Add('   while (xOrder>xRest) do begin xOrder=xOrder-xDivis; end');
...
ErrPos:= '3-'+IntToStr(j)+'-'+IntToStr(TryCount);
            ...
            if not (ResQty>0) then tc.ID2:= cstStarting
              ...
              if (ResQty>tc.Qty) then begin // пересчитывали по кратности
                ...
                WareErrStr:= WareErrStr+fnIfStr(WareErrStr='', '', #13#10)+ss3;
                WaresErrMess:= WaresErrMess+fnIfStr(WaresErrMess='', '', #13#10)+ss3;
              ...
              else WareErrStr:= fnIfStr(WareErrStr='', '', WareErrStr+#13#10)+
...
//            if (E.Message<>'') then
//              WareErrStr:= fnIfStr(WareErrStr='', '', WareErrStr+#13#10)+E.Message;
...
//        if (tc.ID2=cstCutSaved) then tc.ID2:= cstStarting;
    ...
    if (ord.accJoin.ID>0) then begin
      //-------------------------- если unit-счет - добавляем к unit-резерву к/а
// !!! призовые заказы - отдельные счета всегда
//      if (Ord.currID=Cache.BonusCrncCode) then
//        Ord.firma.BonusRes:= Ord.firma.BonusRes+Ord.accJoin.sumlines;

// в какой комментарий писать ???
      //---------------- добавляем комментарий из заказа в WEB-комментарий счета
//      if (Ord.accJoin.webcomm='') then
//        Ord.accJoin.webcomm:= Ord.comment+Ord.commDeliv+Ord.commOrder
//      else begin
...
ErrPos:= '4';
      ...
      if (s1<>'') then ErrorStr:= fnIfStr(ErrorStr='', '', ErrorStr+#13#10)+
        'Ошибка записи комментария в '+sDoc4+' '+Ord.accJoin.Num+' из заказа '+Ord.ORDRNUM
...
// не рано ли писать статус ???
      ibsOrd.SQL.Add(' UPDATE ORDERSREESTR SET ORDRSTATUS='+IntToStr(orstAccepted)+','+
        ' ORDRGBACCCODE='+sAccID+', ORDRGBACCNUMBER="'+Ord.accJoin.Num+'",'+
...
ErrPos:= '5';
      ...
      if (s1<>'') or (Ord.accJoin.recDoc<1) then
        ErrorStr:= fnIfStr(ErrorStr='', '', ErrorStr+#13#10)+
          'Ошибка записи привязки '+sDoc2+' '+Ord.accJoin.Num+' к заказу '+Ord.ORDRNUM;
...
//      if (tc.ID2=cstErrFatal) then Continue; // фатальная ошибка записи
      inc(i);
    end;
...
//      ibsGB.SQL.Add('   from Vlad_CSS_AddAccountHeaderC('+sOrderID+', '+sAccID+', '+
      ...
      ibsGB.SQL.Add(' update PayInvoiceReestr set PINVCLIENTCOMMENT="'+
                    cSpecDelim+'" where PInvCode=:NewAccCode;'); // спец.признак - счет в работе
...
ErrPos:= '6-'+IntToStr(TryCount);
        ...
        on E: Exception do begin
          ...
          if (Pos('lock', E.Message)>0) then
            ...
            if (E.Message<>'') then ErrorStr:= ErrorStr+#13#10+E.Message;
            break;
          end;
          if (TryCount<accRepeatCount) then Sleep(RepeatSaveInterval); // если deadlock, то ждем немного
        end;
...
//------------------------ записали заголовок счета - пишем строки товаров счета
      ...
      ibsGB.ParamCheck:= True;
      if not ibsGB.Transaction.InTransaction then ibsGB.Transaction.StartTransaction;
      ibsGB.SQL.Text:= 'select NewLineCode, ResQty, ResPrice from Vlad_CSS_AddAccLineC('+
...
ErrPos:= '7-'+IntToStr(j)+'-'+IntToStr(TryCount);
            ...
            if not fnNotZero(price) then // проверка 0-й цены
              WareErrStr:= fnIfStr(WareErrStr='', '', WareErrStr+#13#10)+
                           'товар '+wCutName+' записан с 0-й ценой';
...
//---------------------------- записали товары - проверяем ошибки записи товаров
      Success:= true;
      if (ErrCount>0) then begin
        WareErrStr:= WareErrStr+fnIfStr(WareErrStr='', '', #13#10)+
      ...
      if (s1<>'') then ErrorStr:= fnIfStr(ErrorStr='', '', ErrorStr+#13#10)+
        'Ошибка записи комментария в '+sDoc4+' '+sAccID;
      ibsGB.ParamCheck:= True;
...
//--------------------------------------------------- строка закрывающих док-тов
      ...
      if (ord.accJoin.ID<1) then //---------- записываем привязку заказа к счету
        ibsOrd.SQL.Add(' UPDATE ORDERSREESTR SET ORDRSTATUS='+IntToStr(orstAccepted)+','+             // ???
                       ' ORDRGBACCCODE='+sAccID+', ORDRGBACCNUMBER="'+Ord.accSing.Num+'",'+
                       fnIfStr(flSaveCont, ' ORDRCONTRACT='+sContID+',', '')+
                       ' ORDRGBACCTIME="NOW" WHERE ORDRCODE='+sOrderID+';');
      ibsOrd.SQL.Add(' suspend; end');
ErrPos:= '8';
...
//---------------------------------------------------------------- ответ клиенту
    ...
    if (ErrorStr<>'') or (WareErrStr<>'') then fnWriteToLogPlus(ThreadData,
      lgmsSysError, nmProc, ErrorStr, WareErrStr, '', false, 'error');
//      lgmsSysError, nmProc, ErrorStr, WareErrStr, 'ErrPos= '+ErrPos, false, 'error');
    prFreeIBSQL(ibsGB);
    cntsGRB.SetFreeCnt(ibGB);
    prFreeIBSQL(ibsGBt);
    cntsGRB.SetFreeCnt(ibGBt);
  ...
  if flDebug then prMessageLOGS(nmProc+': '+GetLogTimeStr(LocalStart), 'Debug', false); // пишем в log
end;
...
//      if Assigned(Body) then htmpart.Body.Text:= body.Text;    
...
//      if Assigned(Body) then for i:= 0 to Body.Count-1 do   
//        MsgRecive0.Body.Add(Body[i]); // строки сообщения   
//      MsgRecive0.Date:= Now;
      ...
      if Assigned(Attachments) and (Attachments.Count>0) then begin  // прикрепляем файлы
        ...
        for i:= Attachments.Count-1 downto 0 do begin
          ...
          if (j<0) then begin                           // файл не найден
            ...
            htmpart.Body.Text:= htmpart.Body.Text+'<br>'+s;  //vv
//vv            MsgRecive0.Body.Add(s);  
            if RenameFile(Attachments[i], fnTestDirEnd(dir)+fname) then begin
              ...
              htmpart.Body.Text:= htmpart.Body.Text+'<br>'+' - Файл перемещен в папку '+fnGetComputerName+':'+dir+s;     //vv
              htmpart.Body.Text:= htmpart.Body.Text+'<br>'+' Если Вы не получили файл из браузера, сообщите системным администраторам'; //vv
              htmpart.Body.Text:= htmpart.Body.Text+'<br>'+' имена папки и файла из этого письма с запросом о передаче Вам файла по сети.'; //vv
//vv              MsgRecive0.Body.Add(' - Файл перемещен в папку '+fnGetComputerName+':'+dir+s);
//vv              MsgRecive0.Body.Add(' Если Вы не получили файл из браузера, сообщите системным администраторам');
//vv              MsgRecive0.Body.Add(' имена папки и файла из этого письма с запросом о передаче Вам файла по сети.');     
            end;
            Attachments.Delete(i);
...
end;                                     // MessText(mtkUnknownSysType)
...
begin
  ...
  with Cache do try
    ...
    if not Result and (eml='') then
      eml:= 'Не найден E-mail директора по системе учета '+IntToStr(SysID);
  end;
end;  }
...
//        ActText:= wa.Comment;
    ...
    Stream.WriteInt(RestSem); // семафор остатков: 0- красный, 1- желтый, 2- зеленый, другое - нет
...
var iCount, i, j, iSem, iPos, storeMain: integer;
    ...
    for i:= 0 to High(Second) do begin
      ...
      try
        for j:= 0 to OList.Count-1 do with TCodeAndQty(OList[j]) do begin
          if (fnInIntArray(ID, StorageCodes)<0) or (Qty<constDeltaZero) then Continue;
          if (ID=storeMain) then begin
            iSem:= 2; // наличие на сегодня
            break;
          end else iSem:= 1; // наличие на потом
        end;
      finally
        prFree(OList);
      end;
...
if flDebug then
  prMessageLOGS(nmProc+': Sem - '+IntToStr(iSem)+' - Ware.ID= '+IntToStr(Second[i]), fLogDebug, false); // пишем в log

if not flRedesign and (iSem=1) then iSem:= 2;
...
begin
  ...
  try try
    ...
    for i:= 0 to High(Contract.ContStorages) do with Contract.ContStorages[i] do begin
      if not (IsVisible or (flAdd and IsAddVis)) then Continue;
      prAddItemToIntArray(DprtId, StorageCodes);
      if IsDefault then storeMain:= DprtId;
    end;
    ...
    for i:= 0 to OLmarkets.Count-1 do begin
      ...
      try
        for j:= 0 to OList.Count-1 do with TCodeAndQty(OList[j]) do begin
          if (fnInIntArray(ID, StorageCodes)<0) or (Qty<constDeltaZero) then Continue;
          if (ID=storeMain) then begin
            tc.ID2:= 2; // наличие на сегодня
            break;
          end else tc.ID2:= 1; // наличие на потом
        end; // for j:= 0 to OList.Count-1
      finally
        prFree(OList);
      end;
      if (tc.ID2>0) then begin
if not flRedesign and (tc.ID2=1) then tc.ID2:= 2;
        Inc(RestCount);
      end;
...
//========================= получить коды складо контракта и код главного склада
...
begin
  ...
  for i:= 0 to High(Contract.ContStorages) do with Contract.ContStorages[i] do begin
    if not (IsVisible or (flAdd and IsAddVis)) then Continue;
    prAddItemToIntArray(DprtID, StorageCodes);
    if IsDefault then Result:= DprtID;
  end;
end;
//============================ проверка семафоров наличия товаров (Web & WebArm)
...
//  storeMain:= 0;
  try try
    if (ffp.ForFirmID<1) or not Cache.FirmExist(ffp.ForFirmID) // не надо передавать при ForFirmID<1
...
{    Contract:= Cache.arFirmInfo[ffp.ForFirmID].GetContract(ffp.ContID);
    ...
    storeMain:= fnGetContMainStoreAndStoreCodes(ffp.ForFirmID, ffp.ContID, StorageCodes);
...
{      if not Assigned(lst) or (lst.Count<1) then begin
      ...
      for i:= 0 to lst.Count-1 do Strings.Add(lst[i]);
      Strings.Add('');
      Strings.Add('Если Вы не запрашивали восстановление пароля, перешлите это письмо');
      Strings.Add(' в службу поддержки '+Cache.GetConstItem(pcUIKdepartmentMail).StrValue);
    end else begin
      subj:= 'Доступ к сайту ';
...
begin
  ...
  try
    ...
    s:= dprt.CheckShipAvailable(pDate, 0, False, False, False);
    if (s<>'') then raise EBOBError.Create(s);
                                              // границы времен отгрузки на дату
    s:= dprt.GetShipTimeLimits(pDate, TimeMin, TimeMax, flWithSVKDelay, False);
    if (s<>'') then raise EBOBError.Create(s);
  ...
  try
    ...
    case delivType of
      ...
      cDelivClientNow: begin // клиент на складе - только сегодня  ???
        strErr:= dprt.CheckShipAvailable(pShipDate, 0, False, False, False);
      ...
      cDelivSelfGet: begin // Самовывоз
        ...
        try // проверяем время
          ...
          strErr:= dprt.CheckShipAvailable(pShipDate, stID, WithSVKDelay, True, False);
          if (strErr<>'') then raise EBOBError.Create(strErr);
        except
          on E: Exception do begin
            stID:= 0;
      ...
      cDelivTimeTable: try // доставка
        ...
        try
          ...
          strErr:= dprt.CheckShipAvailable(pShipDate, stID, WithSVKDelay, False, True);
          if (strErr<>'') then raise EBOBError.Create(strErr);
        finally
          prFreeIBSQL(ibs);
...
//  if (length(pref)<>10) then Exit; // некорректная длина номера
  ...
  Result:= (Cache.SMSmobileCodes.IndexOf(pref)>=0);
{
...
begin
  ...
  until (c='~');
  if (Cache.SMSmobileCodes.IndexOf(copy(num, i, 3))<0) then Exit; // не нашли код оператора
