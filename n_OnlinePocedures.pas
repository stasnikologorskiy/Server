...
procedure prGetQtyByAnalogsAndStorages_new(Stream: TBoBMemoryStream; ThreadData: TThreadData); //Выдать данные с остатками по склaдам и заказанному кол-ву по товару с аналогами и возможностью редактирования количеств
...
procedure prOrderImport(Stream: TBoBMemoryStream; ThreadData: TThreadData);            // импорт заказа
procedure prGetDestPointParams(Stream: TBoBMemoryStream; ThreadData: TThreadData);     // список параметров для управления торговыми точками (Web)
...
//---------------------------------------------------------- проверка блокировки
    ...
    // блокировка работы конечных пользователей
    ...
    username:= fnCutFIO(Client.Name)+', '+firma.Name+cSpecDelim+firma.UPPERSHORTNAME; // наименование фирмы
...
const nmProc = 'prOrderImport'; // имя процедуры/функции/потока
var OrdIBD: TIBDatabase;
    OrdIBS: TIBSQL;
...
begin
  ...
  ContID:= 0;
  NewOrderID:= 0;
  lstOrd:= TStringList.Create; // список товаров в заказе для проверки дубляжа
  lstOrd.Sorted:= True;
  lstOut:= TStringList.Create; // xml-строки ответа
  lstOut.Capacity:= 1000;
  lstW:= TStringList.Create;   // список товаров для поиска
  lstW.Capacity:= 1000;
  Ncolumns:= 4;
  Setlength(Widths, Ncolumns);
  Widths[0]:= 250; // Наименование
  Widths[3]:= 400; // Результат
  CheckStyle(skTxt);
  CheckStyle(skCTxt);
  CheckStyle(skHead);
  ...
  try
    ...
    FirmId:= Stream.ReadInt;
    ContID:= Stream.ReadInt; // для контрактов
    iCount:= Stream.ReadInt; // принимаем кол-во строк
    ...
    try
      ...
      Client:= Cache.arClientInfo[UserID];
      Contract:= Client.GetCliContract(ContID);
      if (Contract.Status=cstClosed) then raise EBOBError.Create('Контракт '+
        Contract.Name+' закрыт, зайдите в действующий контракт');
      sStore:= IntToStr(Contract.MainStorage);
      ...
      try
        ...
        if (ErrMess<>'') then raise EBOBError.Create(ErrMess);
        ErrMess:= 'Ошибка записи заголовка заказа';
        ...
        //-------------------------------------- готовим список товаров для поиска
        for i:= 1 to High(Cache.arWareInfo) do if Assigned(Cache.arWareInfo[i]) then begin
          Ware:= Cache.arWareInfo[i];
          if not ware.IsWare or (ware=NoWare)              // отсев по наличию
            or ware.IsArchive or (ware.PgrID<1)            // отсев по архивности
            or ware.IsPrize or ware.IsINFOgr               // отсев призов, инфо-товаров
            or (ware.PgrID=Cache.pgrDeliv)                 // отсев доставок
            or (ware.WareState=cWStateInfo) then Continue; // отсев ассортимента
        ...
        AddXmlBookBegin(lstOut);                 //---------------- шапка ответа
        AddXmlSheetBegin(lstOut, 'импорт заказа', Ncolumns, Widths);
        AddStr(sBoldCell('Импорт товаров в заказ '+ORDRNUM));
        AddStr(sHeadCell('Наименование')+sHeadCell('Кол-во')+
               sHeadCell('Заказ')+sHeadCell('Примечание'));

        for i:= 1 to iCount do try //------------------- принимаем строки заказа
          StrIN:= trim(Stream.ReadStr);
          wName:= StrIN;
          sQty:= '0';
          sQty1:= '0';
          //--------------------------------------------------- разбираем строку
          ErrMess:= 'некорректное значение кол-ва';
          pos1:= pos(#9, StrIN);
          if (pos1<1) then raise Exception.Create(ErrMess); // не нащли 2-ю колонку

          wName:= copy(StrIN, 1, pos1-1); // здесь - наименование
          DivMess:= copy(StrIN, pos1+1);
          pos2:= pos(#9, DivMess);
          if (pos2<1) then sQty:= DivMess else sQty:= copy(DivMess, 1, pos2-1);
          sQty:= fnDelEndOfStr(sQty);   // строка вх.кол-ва
          Qty:= StrToFloatDef(sQty, 0);
          if not fnNotZero(Qty) then raise Exception.Create(ErrMess); // некорректное кол-во

          //------------------------------------------- проверяем дубляж товаров
          pos1:= lstORD.IndexOf(wName);
          if (pos1>-1) then
            raise Exception.Create('дубликат товара, измените кол-во в заказе');

          //--------------------------------------------------------- ищем товар
          ErrMess:= 'товар не найден или недоступен';
          pos1:= lstW.IndexOf(wName);
          if (pos1<0) then raise Exception.Create(ErrMess); // не нашли в доступных

          pos2:= Integer(lstW.Objects[pos1]);
          Ware:= Cache.arWareInfo[pos2];
          sWare:= IntToStr(ware.ID); // строка кода товара
          Price:= Ware.SellingPrice(FirmID, Curr, contID);
          if not fnNotZero(Price) then raise Exception.Create(ErrMess); // нет цены

          DivMess:= fnRecaclQtyByDivisible(Ware.ID, Qty); // проверяем кратность
          sQty1:= FloatToStr(RoundTo(Qty, -3)); // строка пересчитанного кол-ва
...
{          OrdIBS.SQL.Text:= 'execute block as declare variable xCode integer; begin'#10+
            ...
            'if (xCode is null or xCode<1) then exception NotCorrect "Ошибка записи";'#10+
          ...
          OrdIBS.SQL.Text:= 'execute block as declare variable xCode integer; begin'#10+
            ...
            'if (xCode is null or xCode<1) then exception NotCorrect "Ошибка записи"; end';
          ErrMess:= RepeatExecuteIBSQL(OrdIBS);
          if (ErrMess<>'') then begin                        // выводим в лог
            fnWriteToLog(ThreadData, lgmsSysError, nmProc, 'wareID='+sWare, ErrMess, '');
            raise Exception.Create('ошибка записи товара');
          end;

          if (DivMess='') then ErrMess:= sCTxtCell(sQty1)+sTxtCell('добавлен')
          else begin
            ErrMess:= sCBoldCell(sQty1)+sTxtGreenCell('добавлен, '+DivMess);
            Inc(DivCount);
          end;
          ...
          on E: EBOBError do raise EBOBError.Create(E.Message); // фатальная ошибка
          on E: Exception do begin                              // ошибка в строке
      ...
      AddXmlBookEnd(lstOut); // отчет готов

// -------------------------------------------------------------- передача в CGI
      Stream.Clear;
      Stream.WriteInt(aeSuccess);  // признак - запрос обработан корректно
      Stream.WriteInt(NewOrderID); // код нового заказа
      Stream.WriteInt(ContID);     // код контракта

      sParam:= sParam+#13#10'NewOrderID='+OrderCode+#13#10'NewOrderNum='+ORDRNUM; // логирование нового заказа

      FName:= GetAppExePath+fnTestDirEnd(DirMailFiles)+'imp_ord'; // папка для файлов
      if not DirectoryExists(FName) then CreateDir(FName); // если нет - создаем

      FName:= fnTestDirEnd(FName)+'imp_ord_'+ORDRNUM+'_'+FormatDateTime('yyyymmdd_hhnnss', Now());
      zipName:= FName+'.zip';
      FName:= FName+'.xml';
      SaveListToFile(lstOut, FName);    // записываем ответ в файл xml

      ErrMess:= ZipAddFiles(zipName, FName); // пакуем в zip
      if (ErrMess<>'') then begin
        fnWriteToLog(ThreadData, lgmsSysError, nmProc, 'ZipAddFiles', ErrMess, '');
      end;
      sleep(10);
      SysUtils.DeleteFile(FName);       // удаляем xml

      Stream1:= TBoBMemoryStream.Create;
      try
        Stream1.LoadFromFile(zipName);
                                 //-------------- передаем файл в браузер
        Stream.WriteStr(ZipContentType);       // тип файла
        Stream.WriteStr(ExtractFileName(zipName));         // имя файла
        Stream.WriteInt(Stream1.Size);         // размер
        Stream.CopyFrom(Stream1, 0);           // файл
      finally
        prFree(Stream1);
      end;
      sParam:= sParam+#13#10'SendFile= true'; // логирование передачи файла

      //--------------------------------------------------------- письмо клиенту
      ErrMess:= ExtractFictiveEmail(Client.Mail);
      if (ErrMess='') then
        sParam:= sParam+#13#10'SendMail= email not found' // логирование отсутствия email
      else begin
        lstW.Clear; // текст письма
        lstW.Sorted:= False;
        lstW.Add('Результаты импорта товаров в заказ '+ORDRNUM+':');
        lstW.Add('--- Принято - '+IntToStr(iCount));
        lstW.Add('--- Добавлено в заказ - '+IntToStr(AddCount));
        if (DivCount>0) then lstW.Add('--- Пересчитано кол-во - '+IntToStr(DivCount));
        if (ErrCount>0) then lstW.Add('--- Отказано - '+IntToStr(ErrCount));
        lstW.Add('Подробнее см. в приложенном файле');

        lstOrd.Clear; // приложение
        lstOrd.Sorted:= False;
        lstOrd.Add(zipName);

        ErrMess:= n_SysMailSend(ErrMess, 'импорт товаров в заказ '+ORDRNUM,
                                lstW, lstOrd, cNoReplayEmail, '', true);
                                        // если не записали в файл для отправки
        ...
        end else sParam:= sParam+#13#10'SendMail= true'; // логирование отправки письма
      end;
    finally
      prSetThLogParams(ThreadData, csOrderImport, UserID, FirmID, sParam); // логирование
    end;
  except
    on E: EBOBError do prSaveCommonError(Stream, ThreadData, nmProc, E.Message, '', True);
    on E: Exception do prSaveCommonError(Stream, ThreadData, nmProc, E.Message, '', False);
...
//          end;
    ...
    Stream.WriteBool(Firm.EnableOrderImport); // разрешение загрузки заказов
...
if flMeetPerson then
      ...
      while not OrdIBS.EOF do begin
        ...
        if (Status=orstForming) then                 // если заказ на стадии формирования -
          Stream.WriteStr(trim(FormatFloat('###0.#', qty))); // передаем текущее кол-во заказа
...
//********************** Добавить строки в заказ из формы проверки наличия (Web)
  ...
  try
    ...
    try
      ...
      for i:= 0 to DataCount-1 do begin //------------------ заполнение массивов
        s:= Stream.ReadStr;            // принимаем <код товара>_<код склада>
        AnalogQty:= Stream.ReadDouble; // принимаем кол-во

        k:= Pos('_', s);                          // нет склада - пропускаем
        if (k<1) then Continue;

        WareID:= StrToIntDef(Copy(s, 1, k-1), 0);   // нет кода товара - пропускаем
        if (WareID<1) then Continue;

        j:= fnInStrArray(IntToStr(WareID), WareCodes); // ищем индекс товара
        if (j<0) then Continue;                   // не нашли - пропускаем

        k:= StrToIntDef(Copy(s, k+1, 10000), 0);  // нет склада - пропускаем
        if (k<1) then Continue;
        if (k<>Contract.MainStorage) then // не глав.склад
          raise EBOBError.Create('Склад проверки не соответствует складу '+
            fnIfStr(OrderExists, 'заказа '+ordnum, 'контракта'));
...
var OrdIBD: TIBDatabase;
  ...
  begin
    ...
    OrdIBS.SQL.Text:= 'select rNewOrderLnCode from AddOrderLineQty'+
                      '('+OrderCode+', '+WareCode+', :WareQty, '+ // код заказа, код товара, ...
...
//============================= для вывода остатков по колонкам пополнения (Web)
procedure prGetQtyByAnalogsAndStorages_new(Stream: TBoBMemoryStream; ThreadData: TThreadData);
const nmProc = 'prGetQtyByAnalogsAndStorages_new'; // имя процедуры/функции
var OrdIBD: TIBDatabase;
    OrdIBS: TIBSQL;
    UserID, FirmID, i, ia, StorageCount, WareCount, WareID, sPos, currID, aCode: integer;
    s, WareCode, OrderCode, ErrPos, PriceFormat, ActTitle, ActText: string;
...
if flNewRestCols then
      try
        flOnlyWare:= Stream.ReadBool;  // только товар (вызов из рез-тов поиска)
      except
        flOnlyWare:= False;
      end;
    finally
      prSetThLogParams(ThreadData, csGetQtyByAnalogsAndStorages, UserID, FirmID,
        'OrderCode='+OrderCode+', WareCode='+WareCode+', WareQty='+FloatToStr(WareQty)+
        #13#10'ContID='+IntToStr(ContID)+#13#10'OnlyWare='+BoolToStr(flOnlyWare)); // логирование
    end;
    ...
    if not OrderExists then  // если заказа нет
      currID:= Cache.arClientInfo[UserID].SearchCurrencyID;  // берем валюту из настроек пользователя
//      Currency:= IntToStr(Contract.DutyCurrency); // если заказа нет, берем валюту из текущего контракта
ErrPos:= '5';

    if not flOnlyWare then ar:= fnGetAllAnalogs(WareID); // аналоги товара

ErrPos:= '6';
    wrba:= TWareRestsByArrive.Create; // объект для колонок остатков по пополнению
    SetLength(wrba.arWares, Length(ar)+1); // 0- код товара, 1... - коды аналогов
    // сначала вход.товар(индекс 0), потом аналоги(индексы от 1)
    ...
    if (s<>'') then raise EBOBError.Create('Ошибка формирования остатков');

    if flOnlyWare and (wrba.arRestLists[0].Count=1)
      and (wrba.arRestLists[0][0]='0') then
      raise EBOBError.Create('Товара нет в наличии');
    ...
    if OrderExists then try
      ...
      while not OrdIBS.Eof do begin
        ...
        owIndex:= fnInIntArray(owID, wrba.arWares); // ищем индекс товара
    ...
    if (arOrderWareQties[0]='0') then // если вход.товара в заказе нет, в строку ввода
      arOrderWareQties[0]:= FormatFloat('###0.#', WareQty); // - вход.кол-во

    Stream.Clear;
    Stream.WriteInt(aeSuccess);
    sPos:= Stream.Position;
    Stream.WriteInt(0); // место под кол-во возвращаемых товаров
    Stream.WriteStr(Cache.GetCurrName(currID, True)); // валюта
    Stream.WriteStr(FormatFloat('###0.#', WareQty));  // вход.кол-во
    Stream.WriteStr(WareCode);                        // код вход.товара
    ...
    for ia:= 0 to High(wrba.arWares) do begin
      ...
      if (WareID<1) then Continue;  // обнулены коды аналогов без остатков
...
//------------------------------------------------- цены Гроссби
      ...
      Stream.WriteStr(wrba.arRestLists[ia][0]); // 1 - кол-во на складе по умолчанию
      Stream.WriteStr(arOrderWareQties[ia]);    // кол-во в заказе
      for i:= 1 to wrba.arRestLists[ia].Count-1 do
        Stream.WriteStr(wrba.arRestLists[ia][i]); // колонки по пополнениям
...
begin
  ...
  try
    ...
    lstBodyMail.Add('отправил заявку на '+sKind+':');
    lstBodyMail.Add('');
...

{
      ...
      if Firm.ShowZeroRests                                         // Ирбис
        and (Version in [cpdHD, cpdAutoHD, cpdMotoHD, cpdMotulHD,
        cpdAutoMotoHD, cpdAutoMotulHD, cpdMotoMotulHD]) then begin  // есть HD
...
//        if (fnInIntArray(Ware.WareBrandID, ForbiddenBrands)>-1) then continue; // пропускаем запрещенные бренды
        if Ware.LoadPriceEx then continue;                                     // пропускаем запрещенные бренды
...
//          +fnIfStr(Firm.ShowZeroRests, ';"Слэш-код"', '');
      Body.Insert(0, s);
      ...
      for i:= 0 to Client.CliContracts.Count-1 do begin
        ...
        if flBlock and (Contract.Status=cstWorked) then begin
          ...
          if firma.SaleBlocked then
            sWarn:= sWarn+fnIfStr(sWarn='', '', ', ')+'Отгрузка запрещена';
        end else stat:= Contract.Status;          // статус контракта для вывода
        ...
        Stream.WriteInt(Contract.ID);            // код контракта
        Stream.WriteStr(Contract.Name);          // номер контракта
        Stream.WriteInt(Contract.PayType);
        Stream.WriteStr(Contract.LegalFirmName); // юр.лицо
        Stream.WriteInt(Contract.DutyCurrency);  // валюта долгов
...
// SaleBlocked=True, WarnMessage<>""  - заблокирован/закрыт - красный фон
// SaleBlocked=False, WarnMessage<>"" - действует/закрыт    - сиреневый фон
        Stream.WriteInt(stat);     // статус контракта
...
//===================== список параметров для управления торговыми точками (Web)
procedure prGetDestPointParams(Stream: TBoBMemoryStream; ThreadData: TThreadData);
const nmProc = 'prGetDestPointParams'; // имя процедуры/функции
...
begin
  ...
  try try
    UserID:= Stream.ReadInt;
    FirmID:= Stream.ReadInt;    // код к/а

    prSetThLogParams(ThreadData, csGetDestPointParams, UserID, FirmID, ''); // логирование
    ...
    if firma.IsFinalClient or (Client.ID<>firma.SUPERVISOR) then
      raise EBOBError.Create(MessText(mtkNotRightExists)); // проверка глав.польз.
...
//---------------------------------------------------------------------- области
    iCount:= 0;
    sPos:= Stream.Position;
    Stream.WriteInt(iCount); // место под кол-во областей
...
//        ' from ADRESSADDPARM where adradadrclasscode=36517 order by adradnamemain';
    IBS.ExecQuery;
    while not IBS.EOF do begin
      Stream.WriteInt(ibs.FieldByName('andtcode').AsInteger); // код области
      Stream.WriteStr(ibs.FieldByName('andtname').AsString);  // название области
      inc(iCount);
      cntsGRB.TestSuspendException;
      IBS.Next;
    end;
    ...
    IBS.ExecQuery;
//--------------------------- тип населенного пункта (село, город, пгт, поселок)
    iCount:= 0;
    sPos:= Stream.Position;
    Stream.WriteInt(iCount); // место под кол-во типов населенных пунктов
    while not IBS.EOF and (ibs.FieldByName('adrcllevel').AsInteger=20) do begin
      Stream.WriteInt(ibs.FieldByName('andtcode').AsInteger);      // код типа нас.пункта
      Stream.WriteStr(ibs.FieldByName('andtname').AsString);       // название типа нас.пункта
      Stream.WriteStr(ibs.FieldByName('adrclshortname').AsString); // сокр.название типа нас.пункта
      inc(iCount);
      cntsGRB.TestSuspendException;
      IBS.Next;
    end;
...
//----------------------------------------------------------- тип инфраструктуры
//--- (бульвар, переулок, площадь, проезд, проспект, спуск, тупик, улица, шоссе)
    iCount:= 0;
    sPos:= Stream.Position;
    Stream.WriteInt(iCount); // место под кол-во типов инфраструктуры
    while not IBS.EOF do begin
      Stream.WriteInt(ibs.FieldByName('andtcode').AsInteger);      // код типа инфраструктуры
      Stream.WriteStr(ibs.FieldByName('andtname').AsString);       // название типа инфраструктуры
      Stream.WriteStr(ibs.FieldByName('adrclshortname').AsString); // сокр.название типа инфраструктуры
      inc(iCount);
      cntsGRB.TestSuspendException;
      IBS.Next;
    end;
