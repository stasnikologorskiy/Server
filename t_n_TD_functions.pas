...
//             функции загрузок из TDT
 ...
 function LoadWareEngLinksUsesFromTDT_24( WareID, UserID: Integer;                       // загрузка связок товара с двигателями из TecDoc (версия 2.4)
...
procedure LoadModelNodeWareUseAndTextListsFromTDT_24(TdtIBS, TdtIBSt, ordIBS, ordIBSr: TIBSQL; // загрузка порций условий и текстов из TecDoc (версия 2.4)
          WareID, pSupMFTD, UserID: Integer; var arInfo: TArLinkInfo;
          ThreadData: TThreadData=nil; CheckTexts: Boolean=False);
...
implementation
uses n_server_common, t_ExcelXmlUse, n_xml_functions, t_ImportChecking, t_WebArmProcedures;         //vv
  ...
  with Cache.FDCA do try try
    ...
    while not TdtIBS.Eof do begin
      ...
      case mfTD of // раскидываем ОН "составных" произв.
       ...
       864: begin // VW-PORSCHE
              setLength(arMFORD, 2); // задаем длину
              arMFORD[0]:= 565; // PORSCHE
              arMFORD[1]:= 587; // VW
            end;
        else begin
...
begin
  ...
  with Cache.FDCA do try try
    ...
    TdtIBS.SQL.Text:= 'select RN_MF_ID mfID, RN_REF_NR Onum,'+
      ...
      ' left join IMPORT_CHANGE_NAMES on ICN_TAB_ID=100 and ICN_KE_KEY=mf_id'+
      ' where RN_ART_NR="'+pArticleTD+'" and ds_mf_id='+IntToStr(pArtSupTD)+
      ' and (mf_pc=1 or mf_cv=1) and mf_del=0 order by mfName, mfID, Onum'; // только произв.авто
    TdtIBS.ExecQuery;
    j:= 0; // счетчик
    while not TdtIBS.Eof do begin
      mfTD  := TdtIBS.fieldByName('mfID').AsInteger;   // код произв.TecDoc
...
{      IBS.ParamCheck:= False;
      ...
      IBS.SQL.Add('  where st.ST_TYP=:xType and st_parent=0 and st_id<>103671'); // 103671 - Мотоцикл
      IBS.SQL.Add('    order by st_descr into :id4, :name4 do begin');
      IBS.SQL.Add('    id3=0; id2=0; id1=0; ga_id=0; name3=""; name2=""; name1=""; ga_name ="";');
...
{
select * from treenodesauto t where t.trnadtsycode = 1 and t.trnavisible = 'T' -- and t.trnatdcode > 0 and t.trnatdga = 'F'
 and ((select count(*) from treenodesauto t2 where t2.trnacodeparent = t.trnacode)>0)
 and ((select count(*) from treenodesauto t1 where t1.trnacodeparent = t.trnacode and t1.trnavisible = 'T')=0)
}
          ...
          if flAdd then begin  // новая нода
            ...
            if flVis and (ParORD>0) then begin
              ...
              if not node.IsVisible then
                raise Exception.Create('нельзя добавить видимый узел в ветку к невидимому родительскому узлу');
            end;
...
begin
  ...
  try try
    ...
    end; // if (TdtIBS.SQL.Text='')
    if not flTD24 then TdtIBS.ParamByName('gaID').AsInteger:= gaID;
...
{
 65364  - MANN BF 1018/1
 372152 - MANN 49 307 55 111
 9769   - MANN BFU 700X = BF 700X
}
end;
...
//=============================== загрузка связок товара с двигателями из TecDoc
function LoadWareEngLinksUsesFromTDT_24( WareID, UserID: Integer;
         var addLinkEng: Integer; ThreadData: TThreadData=nil; CheckArt: Boolean=True): String;
const nmProc = 'LoadWareEngLinksUsesFromTDT_24'; // имя процедуры/функции
var i, j, jj, ii, ij, ji, aCount, uCount, pEngTD, pGA, pLage, pCriTD, pEngORD,
...
begin
  ...
  LocalTime:= Now;
  flSleep:= not flDebug and (fnGetActionTimeEnable(caeOnlyWorkTime) or (High(arInfo)>500));
  try
    ordIBSr.Close;
    ordIBSr.SQL.Clear;
    ordIBS.Close;
  ...
  //------------------------------------------- из TDT порция условий к связке 3
    ...
    for ii:= 0 to High(arInfo) do with arInfo[ii] do begin
      ...
      for jj:= 0 to High(PartLagts) do try // перебираем связки TD
        if (UsesLists[jj].Count>0) then UsesLists[jj].Clear;
        try
          ...
          while not TdtIBS.Eof do begin
            ...
            UsesLists[jj].AddObject(criName+cStrValueDelim+usValue, Pointer(criTD)); // формируем списки условий
  ...
  //------------------------------------------- из TDT порция текстов к связке 3
    ...
    for ii:= 0 to High(arInfo) do with arInfo[ii] do begin
      ...
      for jj:= 0 to High(PartLagts) do begin // перебираем связки TD
        if (TextLists[jj].Count>0) then TextLists[jj].Clear;
        try
          with TdtIBS.Transaction do if not InTransaction then StartTransaction;
          TdtIBS.ParamByName('xLAGT').AsInteger:= PartLagts[jj];
          TdtIBS.ExecQuery;
...
procedure LoadModelNodeWareUseAndTextListsFromTDT_24(TdtIBS, TdtIBSt, ordIBS, ordIBSr: TIBSQL;
          WareID, pSupMFTD, UserID: Integer; var arInfo: TArLinkInfo;
          ThreadData: TThreadData=nil; CheckTexts: Boolean=False);
const nmProc = 'LoadModelNodeWareUseAndTextListsFromTDT_24'; // имя процедуры/функции
// TdtIBS, ordIBS, ordIBSr передаются для увеличения скорости обработки
var ii, criTD, jj, ke, kt, pType, j, i, ResCode, iLagt, iPart, iPartTxt, iDebug: Integer;
    s, ss, criName, usValue, tm: String;
    ErrUseNums, ErrTxtNums: Tai;
    flag, flSleep: Boolean;
    LocalTime: TDateTime;
...
//      prMessageLOGS('begin load supID='+IntToStr(pSupMFTD)+' article '+pArticleTD, 'import_test', False);
      prMessageLOGS('begin load wareID='+IntToStr(WareID)+' warename '+sName, 'import_test', False);
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> связки (не ИНФО-товары)
          if LO.All and not flINFO then try
      ...
      //-------------------------------------------------------- инфо по связкам
            ...
            //------------------------------- порции условий и текстов из TecDoc
              if flTD24 then
                LoadModelNodeWareUseAndTextListsFromTDT_24(TdtIBS, TdtIBSt, ordIBS, ordIBSr,
...
//>>>>>>>>>>>>>>>>>>>>>>>> связи товара с двигателями из TecDoc (не ИНФО-товары)
          if (LO.All or LO.OnlyEng) and not flINFO then begin
            if flTD24 then
              ss:= LoadWareEngLinksUsesFromTDT_24(WareID, UserID, LO.addLinkEng, ThreadData, False)
            else
...
//>>>>>>>>>>>>>>>>>>>>>>>>> файлы рисунков/документов по товару (не ИНФО-товары)
          if (LO.All or LO.OnlyGrafic) and not flINFO then begin
...
//>>>>>>>>>>>>>>>>>>>>> поиск оригинальных номеров товара в TDT (не ИНФО-товары)
    if (LO.All or LO.OnlyON) and not flINFO then try
      ...
      setlength(arWareONums, 0);
    end; // if (LO.All or LO.OnlyON) and not flINFO
    ...
    on E: Exception do begin
      Result:= nmProc+' (ware='+IntToStr(WareID)+'): '+E.Message;
      if (pos('не найден артикул TD', Result)<1) then prMessageLOGS(Result, 'import', False);
    end;
  end;
...
var i, j, MgID, addLink3, addLinkON, istart, iPercent,
    ...
    Ware: TWareInfo;
    pIniFile: TIniFile;
   // OnlyON, OnlyEng, OnlyCross, OnlyCris,
    flag, SendRep, flSelfStart, flAllSups: Boolean;
...
begin
  ...
  try try                       // массив кодов произв. для замены инфо-текстов
    ...
    if istart>-1 then begin //--------------------------------------- загрузка
      ...
      LO.OnlyGrafic:= pIniFile.ReadInteger('reports', 'OnlyGrafic', 0)=1;   // new
      LO.OnlyEAN   := pIniFile.ReadInteger('reports', 'OnlyEAN', 0)=1;      // new
      LO.All:= not LO.OnlyON and not LO.OnlyCross and not LO.OnlyCris and
               not LO.OnlyGrafic and not LO.OnlyEAN;
      LO.WithEng   := LO.All;
        ...
        for i:= istart to High(Cache.arWareInfo) do begin
          if Cache.WareExist(i) then begin
            ...
            if (Ware<>NoWare) and not Ware.IsArchive
              ...
              and (flAllSups or (fnInIntArray(Ware.ArtSupTD, OnlySups)>-1)) then
