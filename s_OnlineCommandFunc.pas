...
function fnfillParametrsAllOrder(Stream: TBoBMemoryStream): string;   //вызывается для заполнения всех данных окна заказа при его открытии
function fnsaveParametrsFromOrder(Stream: TBoBMemoryStream;v: integer;kindSave: integer;deliverydatetext:String; deliverydate: String): string;   //вызывается для сохранения данных окна заказа
...
function fnOpenOrdersListRedisign(Stream: TBoBMemoryStream;ordrcode,warecode,wareqty,inline_,contract,dialogname:String): string;  // возвращает список открытых заказов для запроса от кнопки Заказать
function fnOpenOrdersListQtyRedisign(Stream: TBoBMemoryStream;strPost,strGet:TStringList): string;  // возвращает список открытых заказов для запроса от кнопки Заказать из формы проверки к-ва
function fngetMeetPersonsList(Stream: TBoBMemoryStream;id: String;value: String): string;  //Получение список Встречающих
function fnGetNodeWares_Motul(Stream:TBoBMemoryStream; var userInf:TUserInfo;ScriptName:String): string;
function fnCreateNewUser(Stream:TBoBMemoryStream; var userInf:TUserInfo) :string;// раздел Настройки. Добавить пользователя
function fnGetContractList(Stream:TBoBMemoryStream;edit:String; ContractId:integer) :string;// окно списка Контрактов
function fnGetDestPointParams(Stream:TBoBMemoryStream) :string;// окно выбора торговой точки
...
function fnOpenOrdersList(Stream: TBoBMemoryStream;ordrcode,warecode,wareqty,inline_,contract,dialogname:String): string;  // возвращает список открытых заказов для запроса от кнопки Заказать
var
...
begin
  ...
  try
    ...
    if OrdersCount>0 then   begin
      ...
      s:=s+'<td>Дата заказа</td>';
      s:=s+'<td>Номер заказа</td>';
      s:=s+'<td>Контракт</td>';
      s:=s+'<td>Сумма заказа</td>';
      s:=s+'<td>Валюта</td>';
      s:=s+'<td>Пометка</td>';
      s:=s+'</tr>';
      for i := 0 to OrdersCount-1 do begin
        ...
        s:=s+'<td title="'+fnIfStr(ComentSum<>'', ComentSum, '')+'" style="'+fnIfStr(ComentSum<>'','color: red; font-weight: bold;', '')+'" >'+orderSum+'</td>'; //Сумма заказа
      ...
      s:=s+'<td colspan="6">В новый заказ</td>';
      s:=s+'</tr>';
      s:=s+'</table>';
      s:=' jqswInfoOutOk("Выберите заказ для добавления товара",'''+s+''',"center"); ';
    end
    else
...
begin
  ...
  try
    ...
    if OrdersCount>0 then   begin
      ...
      s:=s+'        <td class="col">Дата заказа</td>';
      s:=s+'        <td class="col">Номер заказа</td>';
      s:=s+'        <td class="col">Контракт</td>';
      s:=s+'        <td class="col">Сумма заказа</td>';
      s:=s+'        <td class="col">Валюта</td>';
      ...
      for i := 0 to OrdersCount-1 do begin
        ...
        s:=s+'<td class="col with-border" title="'+fnIfStr(ComentSum<>'', ComentSum, '')+'" style="'+fnIfStr(ComentSum<>'','color: red; font-weight: bold;', '')+'" >'+orderSum+'</td>'; //Сумма заказа
      ...
      s:=s+'<td class="col" colspan="6">В новый заказ</td>';
      s:=s+'</tr>';
      s:=s+'</table>';
      s:=s+'</div>';
      s:=s+'<a class="close-btn btn info-close" onclick="$(\''#general-info-tree\'').toggleClass(\''hide\'');">Закрыть</a>';
      s:=' jqswfillInfo('''+s+''',"Выберите заказ для добавления товара",5,0,60); '#13#10;
 ...
 begin
  ...
  case v of
   ...
   if kindSave=1 then begin
     s:=s+'jqswConfirmOrderComplete("Заказ отправлен на обработку")'#13#10;
...
begin
  ...
  for i:=0 to StorageCount-1 do begin
    ...
    s:=s+'['+fnIfStr(Storages[i].IsReserve, '1', '0')+', "'+Storages[i].Code+'", "'+StringReplace(Storages[i].FullName, 'СКЛАД', '', [])+'", "'+Storages[i].ShortName+'"]';
  ...
  // s:=s+'var qvColPrice=0;'#13#10;  если включить, колонки    не будут учитываться

  if  (StrToBoolDef(bonus,false)=true) then begin
    s:=s+'s=s+dwwath("'+CurrencyName+'", "'+WareRequestQty+'", "'+WareCodeStr+'","0" ';   // Наименование валюты
    for i:=0 to StorageCount-1 do begin
    ...
    s:=s+'s=s+dwwath("'+CurrencyName+'", "'+WareRequestQty+'", "'+WareCodeStr+'" ,"'+ordr+'"';   // Наименование валюты
    for i:=0 to StorageCount-1 do begin
      s:=s+',"'+Storages[i].FullName+'","'+Storages[i].ShortName+'"';
    end;
    s:=s+');'#13#10;
  ...
  for j:=0 to LineCount-1 do begin
    ...
    WareName:=GetJSSafeStringArg(Stream.ReadStr);        // наименование
    ...
    ActionTitle:=Stream.ReadStr;      // заголовок
    ...
    for i:=0 to StorageCount-1 do begin
      ...
      if WareQtyD>=ChekingQty then begin
        s:=s+',"<b style=''color: green;''>есть</b>"'; // остатки
      end else
            if WareQtyD>8 then begin
              s:=s+',"<b style=''color: red;''>&lt;'+trim(FormatFloat('# ##0.##', ChekingQty))+'</b>"'; // остатки
            end else begin
              s:=s+',"<b style=''color: red;''>'+s2+'</b>"'; // остатки
            end;

           if Storages[i].IsReserve then s:=s+',"'+Stream.ReadStr+'"'; // кол-во в заказе
          end;
      s:=s+');'#13#10;
    end;
    ...
    if bonus='true' then begin
      ...
      s:=s+'  $("#wareanalogdialogbonus").dialog({';//диалог таблицы расписания
      ...
      s:=s+' var f=$("#of table").width();';
      s:=s+'  $("#wareanalogdialog").dialog({';//диалог таблицы расписания
      s:=s+'   width: $("#of table").width()+10 ';
      s:=s+'      });';
      s:=s+'$(''#t1 td'').css(''width'', ''10px'');'#13#10;
      s:=s+'$(''#of'').width($(''#of table'').width());'#13#10;
...
function fnaddLinesToOrder(Stream: TBoBMemoryStream;ScriptName:String;contract:String):String;
 var
  ...
  Storages: TaSD;
begin
 if boolean(Stream.ReadByte) then begin // будут строки
   LineCount:=Stream.ReadInt;
   ...
   for i:=1 to LineCount do begin
     ...
     case Action of
       constOpAdd: begin
         s:=s+'alsto('+IntToStr(Stream.ReadInt) // Код строки заказа
             +', '+Stream.ReadStr              // Код товара
//                         +', '+AccType                     // Код товара
             +', '''+Stream.ReadStr+''''       // Аналог
             +', '''+Stream.ReadStr+''''       // Бренд
             +', '''+Stream.ReadStr+''''       // WareName
             +', '''+Stream.ReadStr+''''       // Колво
             +', '''+Stream.ReadStr+''''       // ЕдИзм
             +', '''+Stream.ReadStr+''''       // Цена
             +', '''+Stream.ReadStr+''''       // Сумма
             +', '''+Stream.ReadStr+'''';      // Сумма баллов
         for j:=0 to StorageCount-1 do begin
// строка снизу - для варианта с отображением количеств на складах
//                         s:=s+', '''+Stream.ReadStr+'''';  // Остатки
           if Storages[j].IsReserve then begin
             s:=s+', '''+Stream.ReadStr+'''';// заказ по складу
           end;
         end;
         s:=s+');'#13#10;
       end; // constOpAdd
       constOpEdit: begin
         s:=s+'elio('+IntToStr(Stream.ReadInt); // Код строки заказа
         Stream.ReadStr;                   // Код товара
         Stream.ReadStr;                   // Аналог
         Stream.ReadStr;                   // Бренд
         Stream.ReadStr;                   // WareName
         s:=s +', '''+Stream.ReadStr+''''       // Колво
              +', '''+Stream.ReadStr+''''       // ЕдИзм
              +', '''+Stream.ReadStr+''''       // Цена
              +', '''+Stream.ReadStr+''''       // Сумма
              +', '''+Stream.ReadStr+'''';      // Сумма баллов
...
//                         s:=s+', '''+Stream.ReadStr+'''';  // Остатки
       ...
       constOpDel: begin
         s:=s+'dlfo('+Stream.ReadStr+');'#13#10; // Код товара
        end; //constOpDel
     end;  // case Action of
   end;
   ...
   s:=s+'elem=$("#totalballs"); if (elem.length) elem.html('''+Stream.ReadStr+''');'#13#10; // новая сумма баллов по заказу
 ...
 end else begin  // надо перейти на новый заказ
...
function fngetMeetPersonsList(Stream: TBoBMemoryStream;id: String;value: String): string;  //Получение список торговых точек
var
...
begin
  ...
  s:=s+'  var s2=''<p style="color: red;">Добавьте мобильный телефон в Настройки > Персональные данные или по тел. '+PhoneSupport+'.</p>'''#13#10;
...
begin
  s:='';
  s:=s+'jqswMessage("Учетная запись для нового пользователя успешно создана. Логин и пароль отправлены на e-mail пользователя.");'#13#10;
...
begin
  ...
  j:=Stream.ReadInt; // основной цикл по товару
  for i := 1 to j do begin
    ...
    Result:=Result+'ImageExt:'''+Stream.ReadStr+''', '; // расширение файла картинки
    ...
    if fsize=0 then begin
      ...
      Result:=Result+'Width:0, '; // размер картинки
      Result:=Result+'Height:0, '; // размер картинки
      Result:=Result+'Image:'''', '
    end
    else begin
      ...
      try
        ...
        try
          ...
          Result:=Result+'Width:'+IntToStr(jpg.Width)+', '; // размер картинки
          Result:=Result+'Height:'+IntToStr(jpg.Height)+', '; // размер картинки
          StreamImg.Clear;
          TMPStream.Clear;
          temp:='';
    ...
    m:=Stream.ReadInt;// количество строк товаров
    Result:=Result+'PLineWaresCount:'+IntToStr(m)+', WaresTableData:new Array()};'#13#10;
    for k := 1 to m do begin // цикл самих товаров
      ...
      Result:=Result+''', ActionImageExt:'''+Stream.ReadStr+''', '; // расширение файла картинки иконки
      fsize:=Stream.ReadInt;
      Result:=Result+'ActionImageSize:'+IntToStr(fsize)+', '; // размер картинки   иконки
      if fsize=0 then begin
