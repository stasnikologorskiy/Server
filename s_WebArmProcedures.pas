unit s_WebArmProcedures; // процедуры для Web
...
function fnGetWebArmDirModelPage(PageType: byte; PageName:String; Stream: TBoBMemoryStream): string; // страница узлов мотоциклов/авто


const
  StandartErrorMessage        : string = 'Произошла ошибка выполнения. Если эта ошибка происходит регулярно, сообщите об этом по адресу ';
...
// рисует форму поиска+ всякие кнопки для подбора и т.п.
...
begin
  ...
  Result:=Result+'<input type=text id=waresearch name=waresearch insearchtext="Поиск товара" maxlength=32 value="" onfocus="waresearchfocus(this);" onblur="waresearchblur(this);"  style="width: 136px;">';
  ...
  Result:=Result+'<a class="abgslide" style="width: '+IntToStr(width)+'px; height: 22px; right: -'+IntToStr(Shift)+'px; top: 0px; background-image: url('''+fnIfStr(userInf.FirmID=IntToStr(isWe), DescrImageUrl, '')+'/images/combobox.png'');" '
                +'href="#"  title="Последние поисковые запросы по товарам" onclick=''var auto=$("#waresearch"); if (auto.attr("opened")=="true") {auto.autocomplete("close")} else {auto.autocomplete("search" , "");} auto[0].focus();''></a>';
...
//                  +'href="#" title="Поиск" onclick="$(''#searchform'').trigger(''submit'');"></a>';
                  +'href="#" title="Поиск товара" onclick="ws();"></a>';
  ...
  if OE then begin
    ...
    width:=136;  Shift:=Shift+width+3+4+12; // 4 - это всякие margin и border input`a, 12 - отступ от поиска по товару
    Result:=Result+'<input type=text id=vinsearch name=vinsearch insearchtext="Поиск по VIN" maxlength=17 value="" onfocus="waresearchfocus(this);" onblur="waresearchblur(this);"  style="position: absolute; top: 0px; right: -'+IntToStr(Shift)+'px; width: '+IntToStr(width)+'px;">';
    Result:=Result+'<input type=submit style=''display: none;''>';
    ...
    width:=19;  Shift:=Shift+width-1; //
    Result:=Result+'<a class="abgslide" style="width: '+IntToStr(width)+'px; height: 22px; right: -'+IntToStr(Shift)+'px; top: 0px; background-image: url('''+fnIfStr(userInf.FirmID=IntToStr(isWe), DescrImageUrl, '')+'/images/combobox.png'');" '
                  +'href="#"  title="Последние поисковые запросы по VIN" onclick=''var auto=$("#vinsearch"); if (auto.attr("opened")=="true") {auto.autocomplete("close")} else {auto.autocomplete("search" , "");} auto[0].focus();''></a>';

    width:=23;  Shift:=Shift+width+3; //
    Result:=Result+'<a class="abgslide" style="width: '+IntToStr(width)+'px; height: 22px; right: -'+IntToStr(Shift)+'px; top: 0px; background-image: url('''+fnIfStr(userInf.FirmID=IntToStr(isWe), DescrImageUrl, '')+'/images/lupa.png'');" '
                  +'href="#" title="Поиск по VIN" onclick="$(''#searchbyvinform'').submit();"></a>';
  end;
...
(*
  ...
  Result:=Result+'<a id="selbymodauto" class="abgslide" style="width: 24px; height: 24px; right: -'+IntToStr(Shift)+'px; top: -1px; background-image: url(''/images/autoicon.png'');" '
                +'href="#" onclick="ec(''openselbymodwindow'', ''selname=manuflistauto&sys='+IntToStr(10+constIsAuto)+'&id=-1&objdiv=selbymodelautoobj&top10=autotop10'', ''difdict'');" title="Подбор по модели автомобиля"></a>';
  ...
  Result:=Result+'<a id="selbymodengine" class="abgslide" style="width: 24px; height: 24px; right: -'+IntToStr(Shift)+'px; top: -1px; background-image: url(''/images/engineicon.png'');" '
                +'href="#" onclick="ec(''openselbymodwindow'', ''selname=manuflistautoengine&sys='+IntToStr(30+constIsAuto)+'&id=-1&objdiv=selbymodelauenobj&top10=auentop10'', ''difdict'');" title="Подбор по двигателю автомобиля"></a>';
  ...
  Result:=Result+'<a id="selbymod" class="abgslide" style="width: 101px; height: 24px; right: -'+IntToStr(Shift)+'px; top: -1px; background-image: url(''/images/alloftheworld.png'');" '
                  +'href="#" onclick="ec(''openselbymodwindow'', ''selname=manuflist&sys='+IntToStr(10+constIsMoto)+'&id=-1&objdiv=selbymodelmotoobj&top10=mototop10'', ''difdict'');" title="Подбор по типу транспортного средства"></a>';
  ...
  Result:=Result+'<a id="selbyattr" class="abgslide" style="width: 24px; height: 24px; right: -'+IntToStr(Shift)+'px; top: -1px; background-image: url(''/images/rule.png'');" '
                +'href="#" title="Подбор детали по параметрам"></a>';

  Shift:=Shift+34;
//  Result:=Result+'<input type=button style=''position: absolute; right: -'+IntToStr(Shift)+'px; top: -1px; width: 31px;'' value="OE" onclick=''$.fancybox.open($("#origprogs"), {"modal" : false, "padding": 0});$("#oetopdiv").css("top", "0px");'' title="Подбор по оригинальным программам">';
  Result:=Result+'<input type=button style=''position: absolute; right: -'+IntToStr(Shift)+'px; top: -1px; width: 31px;'' value="OE" onclick=''tuneheader();'' title="Подбор по оригинальным программам">';
*)


  width:=46;  Shift:=Shift+width+3+12; //  12 - отступ от поиска по товару или VIN
  ...
  ' href=# onclick=''$("#podbortabs").dialog("open");'' title="Все подборы в одном окне"></a> ';

//  Shift:=Shift+79;
//  Result:=Result+'<input type=button value="Сравнить" disabled id=warecomparebtn style=''position: absolute; right: -'+IntToStr(Shift)+'px; top: -1px; width: 76px;'' onclick=''warecompare();''>';

  width:=70;  Shift:=Shift+width+3;
  if flNewModeCGI then
    Result:=Result+'<input type=button id=deliverieslistbtn style=''position: absolute; right: -'+IntToStr(Shift)+'px; top: -1px; width: '+IntToStr(width)+'px; display: none;'' value="Доставки" onclick=''ec("getdeliverieslist", "priceinuah="+$("#invoicecurrency").val, "newbj");'' title="Список доставок для добавления в счет">'
  else
    Result:=Result+'<input type=button id=deliverieslistbtn style=''position: absolute; right: -'+IntToStr(Shift)+'px; top: -1px; width: '+IntToStr(width)+'px; display: none;'' value="Доставки" onclick=''ec("getdeliverieslist", "priceinuah="+$("#invoicecurrency").val, "abj");'' title="Список доставок для добавления в счет">';
  Result:=Result+'</div>';
...
//leftform - рисуется ли форма на левой панели
begin
  Result:='';
...
//  Result:=Result+'<form  action="'+Request.ScriptName+'/anbj" onsubmit="alert(''111''); return false;">';
  ...
  Result:=Result+'<nobr>логин&nbsp;<input type=text class=input1 name=psw '+fnIfStr(leftform, 'id=leftpsw', '')+'></nobr><br>';
  Result:=Result+'<nobr>пароль&nbsp;<input type=password class=input1 name=lgn></nobr><br>';
//  Result:=Result+'<a href='+Request.ScriptName+'/registration>регистрация'+GetQueryField(Request, 'ttt')+'</a>';
  if leftform then begin
    Result:=Result+'<input type=submit class=input1 style=position: relative;" value="Вход">';
    Result:=Result+'<input type=hidden name=leftform value=true>';
    Result:=Result+'</form>';
    Result:=Result+'<br/><br/><input type=button class=input1 style="position: relative; left: 1px; width: auto;" value="Восстановить пароль" '
                +'onclick=''if (mtrim($("#leftpsw").val())=="") {alert("Вы не указали свой логин")} else {ec("restorepass", "login="+mtrim($("#leftpsw").val()), "anbj");}''><br>';
  end else begin
    Result:=Result+'<input type=submit class=input1 style="display: none;" value="Вход">';
    Result:=Result+'</form>';
  end;
  Result:=Result+'</div>';
end;
...
begin
  ...
  if OE then begin
    Result:=Result+'    <li><a href="#origprogs" title="Подбор по оригинальным программам">Оригинальные каталоги</a></li>'#13#10;
  end;
  if Auto then begin
    Result:=Result+'    <li><a href="#selectbymodelautodiv" title="Подбор по модели автомобиля">Авто</a></li>'#13#10;
    Result:=Result+'    <li><a href="#selectbyengineautodiv" title="Подбор по двигателю автомобиля">Двигатель</a></li>'#13#10;
    Result:=Result+'    <li><a href="#autolampdiv" title="Подбор по автоламп по цоколю">Лампы</a></li>'#13#10;

  end;
  if Moto then begin
    Result:=Result+'    <li><a href="#selectbymodelmotodiv" title="Подбор по модели мотоцикла">Мото</a></li>'#13#10;
    Result:=Result+'    <li><a href="#selectbyattributemotodiv" title="Подбор по параметрам">По параметрам</a></li>'#13#10;
  end;
  Result:=Result+' </ul>'#13#10;
  ...
  if OE then begin
    ...
    Result:=Result+'<div id="origprogsheader" class="selectpartsdivheader"><h1 class="grayline" style="height: 26px; padding: 14px 0 0; width: 100%;">'
            +'<a href=# style="vertical-align: 75%; margin-left: 78px;" class="origyears" onclick=''setpodborsubdiv(0, -1);''>НАЧАЛО</a></h1>';
    Result:=Result+'<img src="/images/back.png" style="top: 2px;" class="backimage" title="Назад" onclick=''setpodborsubdiv(-1, -1);''>';
...
//    Result:=Result+laximostartpage;
  ...
  if Moto then begin
  // ++++++++++++++++++++++++++ блок подбора по атрибутам мото +++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    Result:=Result+'<h1 class="grayline" style="margin: 0px;">Введите значения для подбора</h1>';
    Result:=Result+'<img src="/images/back.png" class="backimage" title="К списку групп аттрибутов" '
  ...
  // ++++++++++++++++++++++++++ блок подбора по модели мото +++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    Result:=Result+'<tr><td>Производитель: </td><td><select id=manuflist     style="width: 550px; margin-right: 20px;" onchange="loadmodellinelist(this, ''modellisttable'', ''modellinelist'', '+IntToStr(constIsMoto)+');"></select></td></tr>';
    Result:=Result+'<tr><td>Модельный ряд: </td><td><select id=modellinelist style="width: 550px; margin-right: 20px;" onchange="loadmodelslist(this, ''modellisttable'', '+IntToStr(constIsMoto)+');"></select></td></tr>';
    ...
    Result:=Result+'<div  style="text-align: left; height: 220px; width: 100%; overflow-y: auto;" >'; // +++ див для топ10
    Result:=Result+'<h1 class="grayline" style="margin-bottom: 0px;">Последние модели</h1>';
    ...
    Result:=Result+'<h1 class="grayline" id=motomodeltreeheader style=''height: 32px;''>Двойной клик по узлу - переход к отображению товаров</h1><br/>';
    Result:=Result+'<img src="/images/back.png" class="backimage" title="К списку моделей" '
                  +'onclick="setpodborsubdiv(-1, -1); ">';  //
    Result:=Result+'</div>';
  ...
  // ++++++++++++++++++++++++++ блок подбора по модели авто +++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    Result:=Result+'<tr><td>Производитель: </td><td><select id=manuflistauto     style="width: 550px; margin-right: 16px;" onchange=''loadmodellinelist(this, "modellisttableauto", "modellinelistauto", '+IntToStr(constIsAuto)+');''></select></td></tr>';
    Result:=Result+'<tr><td>Модельный ряд: </td><td><select id=modellinelistauto style="width: 550px; margin-right: 16px;"  onchange=''loadmodelslist(this, "modellisttableauto", '+IntToStr(constIsAuto)+');''></select></td></tr>';
    Result:=Result+'</table><div style="height: 200px; width: 100%; overflow-y: auto;" > ';
    ...
    Result:=Result+'<div  style="text-align: left; height: 220px; width: 100%; overflow-y: auto;" >'; // +++ див для топ10                                                                                                                        Result:=Result+'<h1 class="grayline" style="margin-bottom: 0px;">Последние модели</h1>';
...
//                  +'onclick="showseldiv(''selbymodeldivauto'', ''selbymodelautoobj''); ">';  //
  ...
  // ++++++++++++++++++++++++++ блок подбора по двигателю авто +++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    if flNewModeCGI then
      Result:=Result+'Производитель: <select id=manuflistautoengine style="width: 350px; margin-right: 16px;" onchange=''$("#listautoengine")[0].options.length=0; if (this.value!=-1) ec("loadengines", "id="+this.value, "newbj");''></select><br />'
    else
      Result:=Result+'Производитель: <select id=manuflistautoengine style="width: 350px; margin-right: 16px;" onchange=''$("#listautoengine")[0].options.length=0; if (this.value!=-1) ec("loadengines", "id="+this.value, "abj");''></select><br />';
    Result:=Result+'<span style=''margin-right: 72px;''>Двигатель . . . Объем . . . кВт . . . л.с. . . . К-во цилиндров</span><br />';
    Result:=Result+'Двигатель: <select id=listautoengine style="width: 350px; margin-right: 16px;"></select><br />';
    if flNewModeCGI  then
  ...
  (*
    Result:=Result+'<table style="text-align: center; font-weight: bold; background-image: url(''/images/gray_line.png'');"><tr><td width=125>Двигатель</td><td width=57>Объем</td><td width=74>кВт</td><td width=74>л.с.</td><td width=132>К-во цилиндров</td></tr>';
    Result:=Result+'</table>';
  ...
  *)
    ...
    Result:=Result+'<div  style="text-align: left; height: 250px; width: 100%; overflow-y: auto;" >'; // +++ див для топ10
    Result:=Result+'<h1 class="grayline" style="margin-bottom: 0px;">Последние выбранные</h1>';
    ...
    Result:=Result+'<img src="/images/back.png" class="backimage" title="К списку двигателей" '
                  +'onclick="setpodborsubdiv(-1, -1); ">';  //
...
// Replace values for the template plugin';
  ...
  if (userInf.PageName='accountsreestr') then begin
    Result:=Result+'  $("#contractdatadiv").dialog({ autoOpen: false, title:"Контракт", resizeStop: savefloatingwinparams, dragStop: savefloatingwinparams, open: readfloatingwinparams});'#13#10;
...
//    Result:=Result+'  if ($("#divtypetree").length) {$("#divtypetree").dialog({autoOpen: false, width:500});};'#13#10;
  ...
  Result:=Result+'<div id="jqdialog" style="display: none;"></div>'; // див под диалоги jQuery
  Result:=Result+'<div id="jqdialoginfo" style="display: none;"></div>'; // див под диалоги jQuery
  //Result:=Result+'<div id="jqdialoginfocreate" style="display: none;"></div>'; // див под диалоги jQuery
  ...
  Result:=Result+'</div>';
  Result:=Result+'<div style="position: absolute; left: -200000px;">'#13#10; // чисто спрятать внутренние дивы, а когда их fancybox начнет показывать, hide с них уйдет
  ...
  if Autenticated  then begin
    ...
    if userInf.PageName='accountsreestr' then begin
      Result:=Result+'<div id=warerestsdiv defwidth=250 defheight=400 defleft=2 deftop=715><form action="'+ScriptName+fnIfStr(flNewModeCGI,'/newbj','/abj')+'" onsubmit="return false;"><input type=hidden id=warecodeforinvoice><div></div><input type=text size=6 maxlength=6 id=wareqtyforinvoice>'
                    +'<button onclick=''ec("addwaretoinvoice", "wareid="+$("#warecodeforinvoice").val()+"&forfirmid="+$("#forfirmid").val()+"&accid="+$("#invoicecode").val()+"&wareqty="+$("#wareqtyforinvoice").val(),'+fnIfStr(flNewModeCGI,'"newbj"','"abj"')+');'' id=btnwaretoinvoice>В счет</button></form></div>'#13#10;
...
//      Result:=Result+'Кредитный лимит: <span id=credlimit></span><br>Отсрочка платежа: <span id=paydelay></span> дней<br><span id=debtsum></span><br> Резерв: <span id=reserv></span><br>';
//      Result:=Result+'<span id=blockingmessage></span>';
    ...
    if userInf.PageName='import' then begin
      Result:=Result+'<div id=divmodeltable style="width: 500px; margin: 10px; overflow: hidden;" title="Модель"><table id=modeltable style="cursor: default;" ></table></div>'#13#10;
      Result:=Result+'<div id=divautotree style="width: 1000px; margin: 10px; overflow: auto;" class="treeview" title="Узлы"><ul id=rep_auto_ul_0  style=''position: relative;''></ul></div>';
    end;
    ...
    if (fnInIntArray(rolProduct, userInf.Roles)>-1) then begin
      ...
      if flNewModeCGI then
        Result:=Result+'<input type=button value="Добавить" onclick=''ec("addornum", "ornumware="+$("#addornumbyhand").attr("name")+"&ornumtext="+$("#ornumtext_addornum").val()+"&manufacturerid="+$("#manufacturerid_addornum").val(), "newbj");''>'
      else
        Result:=Result+'<input type=button value="Добавить" onclick=''ec("addornum", "ornumware="+$("#addornumbyhand").attr("name")+"&ornumtext="+$("#ornumtext_addornum").val()+"&manufacturerid="+$("#manufacturerid_addornum").val(), "pabj");''>';
    ...
    if userInf.PageName='mppregord' then begin
      ...
      Result:=Result+'<tr>';
      Result:=Result+'  <td style="text-align: right;">Логин: </td>';
      Result:=Result+'  <td><input type=text maxlength=20 name=login id=selpersonlogin value=""></td>';
      Result:=Result+'</tr>';

      Result:=Result+'<tr>';
      Result:=Result+'  <td style="text-align: right;">Контрагент: </td>';
      ...
      Result:=Result+'<tr>';
      Result:=Result+'  <td style="text-align: right;">Должн. лицо: </td>';
      Result:=Result+'  <td><SELECT style="width: 400px;" name=person id=selperson></select>';
      Result:=Result+'</tr>';
{
      Result:=Result+'<tr>';
      Result:=Result+'  <td style="text-align: center;" colspan=2><input type=submit value="Выполнить заявку">&nbsp;<input type=button value="Закрыть" onclick="$.fancybox.close();"></td>';
      Result:=Result+'</tr>';
}
        Result:=Result+'</table>';
        Result:=Result+'</form>';
  ...
  if userInf.NeedTinyMCE then begin
    ...
    Result:=Result+'    <div id="selectimagesdivcaption">Допустимый размер изображения:</div>';
    Result:=Result+'    <div id="selectimagesdiv">Список изображений';
    ...
    Result:=Result+'<div class=imgloaddiv><form method="GET" enctype="multipart/form-data" '+
                   ...
                   '</div>';
                  // '<input type=button id=waredelbtn value=''Удалить изображение'' onClick=''ecq("delwareimg", "editor=tinyeditor&wareid="+$(".imgloaddiv #wareid")[0].value, "difdict", "Вы действительно хотите удалить изображение?");''></div>';
    Result:=Result+'  </div>';
    ...
    Result:=Result+'  <input id=tesavebtn type=submit value="Сохранить">'
                  +'&nbsp;<input type=button value="Закрыть" onclick="$(''#TinyEditDIV'').css(''display'', ''none'')"></form>';
//    Result:=Result+'  <input id=tesavebtn type=button value="Сохранить" onclick=''ec("savewareinfo", "id="+this.name+"&text="+tinyserialize(tinyMCE.get("tinyeditor").getContent(), "tinyeditor"), "difdict");''>'
    Result:=Result+'  </div>';
    Result:=Result+'</div>';
  end;
// -------------------------- tinyMCEEditor ---------------------------------------------------
  if userInf.NeedTinyMCEAction then begin
    ...
    Result:=Result+'  <div id="header-action-div">'+
      '<span id="action-header-caption">Заголовок акции:</span><input type=text size="80" id="action-header" name="action-header" value="">'+
      '<br><br>'+
      '<span class="action-check-span">Связанные продукты:</span><input id="action-is-plex" name="action-is-plex" type=checkbox  disabled="disabled" title="" >'+
      '<span class="action-check-span">Связанные CIPHER: </span><input id="action-is-chex" name="action-is-chex" type=checkbox  disabled="disabled" title="" >'+
      '<span class="action-join-span">Связанная акция: </span><select id="join-action-select" onchange="changeMotulJoinWaresAction();"></select>'+
    '</div>';
    Result:=Result+'  <div id="header-action-date-div">'+
     '<span class="action-date-span">Дата начала: </span><input id="action-fromdate" name="action-fromdate" maxlength="8" size="8" value="" type="text">'+
     ...
     '<span class="action-date-span">Дата окончания: </span><input id="action-todate" name="action-todate" maxlength="8" size="8" value="" type="text">'+
    ...
    Result:=Result+'  <div id=tebottompanel>';
    Result:=Result+'  <input id=tesavebtn type=submit value="Сохранить">'
                  +'&nbsp;<input type=button value="Закрыть" onclick="$(''#TinyEditActionFilesDIV'').css(''display'', ''none'')"></form>';
    Result:=Result+'  </div>';
    Result:=Result+'<div class="wareinfoimgdiv">';
    Result:=Result+'    <div class="selectimagesdivcaption">Текущее изображение товара:</div>';
    ...
    Result:=Result+'   <div class="imgloaddiv">'+
                   ...
                   '   if ($(''#ffile-image'').attr(''value'')=='''') { alert(''Вы не указали файл''); $(''#ffile'')[0].focus();} '+
                   '   return sendfile(this);">'+
...
// -------------------------- tinyMCEEditorAction ---------------------------------------------------
  ...
  if Autenticated then begin
    ...
    if Autenticated then begin
      ...
      if (userInf.ShowImportPage)
        then Result:=Result+fnDrawMenuItem('Импорт' , 'import',  'import', 'Страница импорта из файлов', '');
      if (fnInIntArray(rolProduct, userInf.Roles)>-1) or (fnInIntArray(rolUiK , userInf.Roles)>-1) then
        Result:=Result+fnDrawMenuItem('Товары' , 'wares',  '', '', '');
      if (fnInIntArray(rolOPRSK, userInf.Roles)>-1) then Result:=Result+fnDrawMenuItem('Счета' , 'accountsreestr',  '', '', '');
      if (fnInIntArray(rolOPRSK, userInf.Roles)>-1) then Result:=Result+fnDrawMenuItem('Заказы клиентов' , 'order',  '', '', '');
      if ((fnInIntArray(rolRegional, userInf.Roles)>-1) or (fnInIntArray(rolManageSprav, userInf.Roles)>-1) or (fnInIntArray(rolUiK, userInf.Roles)>-1)  or (fnInIntArray(rolCustomerService, userInf.Roles)>-1)) then Result:=Result+fnDrawMenuItem('Контрагенты' , 'mpp',  'mpp', 'Управление пользователями', '');
      if ((fnInIntArray(rolModelManageAuto, userInf.Roles)>-1) or (fnInIntArray(rolModelManageMoto, userInf.Roles)>-1)) then Result:=Result+fnDrawMenuItem('Применимость' , 'cou',  'COUPage', 'Управление условиями применимости', '');
      ...
      if (fnInIntArray(rolRegional, userInf.Roles)>-1)
        ...
        then Result:=Result+fnDrawMenuItem('Заявки' , 'mppregord',  'mppregord', 'Заявки на регистрацию', '');

      if (fnInIntArray(rolSaleDirector, userInf.Roles)>-1)
        then Result:=Result+fnDrawMenuItem('Области' , 'mppregzones',  'mppregzones', 'Зоны обслуживания', '');


      if (fnInIntArray(rolManageUsers, userInf.Roles)>-1) then Result:=Result+fnDrawMenuItem('Пользователи' , 'webarmusers',  'webarmusers', 'Управление webarm-пользователями', '');
      if (fnInIntArray(rolManageBrands, userInf.Roles)>-1) then Result:=Result+fnDrawMenuItem('Бренды' , 'brandcross',  'brandcross', 'Управление соответствиями брендов GrossBee и TecDoc', '');
      if (fnInIntArray(rolManageBrands, userInf.Roles)>-1) then Result:=Result+fnDrawMenuItem('Логотипы' , 'logotypes',  'logotypes', 'Управление отображением логотипов и переходом по ссылкам производителей', '');
      if (fnInIntArray(rolTNAManageAuto, userInf.Roles)>-1) then Result:=Result+fnDrawMenuItem('Узлы автомобиля' , 'treeauto',  'TNAManagePageAuto', 'Управление справочником узлов автомобиля', '');
      if (fnInIntArray(rolTNAManageMoto, userInf.Roles)>-1) then Result:=Result+fnDrawMenuItem('Узлы мотоцикла' , 'treemoto',  'TNAManagePageMoto', 'Управление справочником узлов мотоцикла', '');
      if flMotulTree then
        if (fnInIntArray(rolTNAManageMotul, userInf.Roles)>-1) then Result:=Result+fnDrawMenuItem('Узлы Motul' , 'treemotul',  'TNAManagePageMotul', 'Управление справочником узлов Motul', '');
      if (fnInIntArray(rolModelManageAuto, userInf.Roles)>-1) then Result:=Result+fnDrawMenuItem('Спр. мод. авто' , 'dirmodauto',  'prDirModelPageAuto', 'Управление справочником производителей/модельных рядов/моделей автомобилей', '');
      if (fnInIntArray(rolModelManageMoto, userInf.Roles)>-1) then Result:=Result+fnDrawMenuItem('Спр. мод. мото' , 'dirmodmoto',  'DirModelPageMoto', 'Управление справочником производителей/модельных рядов/моделей мотоциклов', '');

      if (fnInIntArray(rolNewsManage, userInf.Roles)>-1) then Result:=Result+fnDrawMenuItem('Новости' , 'newsmanage',  'newsmanage', 'Управление акциями и прочим', '');
      if (fnInIntArray(rolNewsManage, userInf.Roles)>-1) then Result:=Result+fnDrawMenuItem('Уведомления' , 'notifications',  'notifications', 'Управление уведомлениями', '');


      //if (fnInIntArray(rolUiK, userInf.Roles)>-1) then Result:=Result+fnDrawMenuItem('УиК' , 'uik',  'uik', 'Страница с инструментарием отдела Учета и Контроля', '');

      if (userInf.SysOptions)
        then Result:=Result+fnDrawMenuItem('Настройки системы' , 'sysoptions',  'sysoptions', 'Страница насторек системы', '');
      Result:=Result+fnDrawMenuItem('Настройки' , 'options',  'options', 'Настройки', '');
      if ((fnInIntArray(rolManageSprav, userInf.Roles)>-1) or (fnInIntArray(rolUiK, userInf.Roles)>-1)) then
        Result:=Result+fnDrawMenuItem('Информация' , 'info',  'info', 'Информация', '');
      if ((fnInIntArray(rolRegional, userInf.Roles)>-1) or (fnInIntArray(rolSuperRegional, userInf.Roles)>-1) or (fnInIntArray(rolSaleDirector, userInf.Roles)>-1)) then  //vv
        Result:=Result+fnDrawMenuItem('Отчеты ВI' , 'mpbireports',  'mpbireports', 'Страница импорта из файлов', '');  //vv
      if (fnInIntArray(rolManageMotulSite, userInf.Roles)>-1) then
        Result:=Result+fnDrawMenuItem('motul.vladislav.ua' , 'motulsite&kindofpage=3',  'motulsite', 'Страница управления содержимым сайта motul.vladislav.ua', '');
      if flNewModeCGI then begin
        Result:=Result+fnDrawMenuItem('Выход' , 'quit',  'quit', '', 'ec("quit", "", "newbj")');
      end
      else begin
        Result:=Result+fnDrawMenuItem('Выход' , 'quit',  'quit', '', 'ec("quit", "", "anbj")');
      end;
    end else begin
 ...
 Result:=Result+'<iframe id="downloadframe" style="visibility: hidden;"></iframe>'; // фрейм для загрузки файлов
  ...
  Result:=Result+'  <h1>Заголовок поиска</h1>'#13#10;
 ...
 if (userInf.PageName='accountsreestr')  then begin
   ...
   Result:=Result+'<div id="deliverysheduler_viewall" ><button id=showalldeliverybtn title="Показать доступные отправки" Onclick="ShowAllSchedulerDelivery();">'+
  ...
  Result:=Result+'<fieldset><legend>Способ получения товара</legend>';
  Result:=Result+'<input name=typeofgetting type=radio id=getting1 value=1 checked title=""><span title="Заказываемый товар будет зарезервирован для Вас на складе отгрузки">Резерв </span>';
  Result:=Result+'<input name=typeofgetting type=radio id=getting2 value=2 title=""><span title="Заказываемый товар будет собран для самовывоза к указанным дате и времени">Самовывоз</span> ';
  ...
  Result:=Result+'<td>Дата: </td><td><select name=deliverydate  onChange="checkWareOnStorage();  ';
  ...
  Result:=Result+'<td><span id=pickuptimespan> Время: <select name=pickuptime  onClick=" fillHelpDesk(); '+
  ...
  Result:=Result+'<tr><td><span id=ttCaption>Торговая точка:</span></td><td><select id=_ttorderselect name=tt _deliverykind="-1" _deliverytimeout="-1" _deliverytimein="-1" _shedulercode="-1" onClick="fillHelpDesk(); checkWareOnStorage(); ';
  ...
  Result:=Result+'    <button disabled="disabled" id=showdeliveriesbtn title="Показать доступные отправки" style=" font-color:#000000;"  onClick=" fillHelpDesk(); if (!this.disabled) {'+
  ...
  Result:=Result+'<tr class="grayline"><td align="left">Время отгрузки со склада</td> <td align="left" >Способ доставки</td><td align=left" >Планируемое прибытие</td>';
  Result:=Result+'</tr>';
  Result:=Result+'<tr >';
  ...
  prOnReadyScriptAdd('  InitUIDialog(''deliveryshedulerdiv'',''Выберите расписание'',500); '#13#10);
  prOnReadyScriptAdd('  DeliveryFormEvent();  '#13#10);

 end;


  // Для редактирования нововсти
  Result:=Result+'<div id=conteinernewseditdiv ></div>'#13#10;    //окно вывода редактирования новости
  prOnReadyScriptAdd('  InitUIDialog(''conteinernewseditdiv'',''Редактирование новости'',800); '#13#10);

  Result:=Result+'<div id=warenodefilterdialog ></div>'#13#10;    //окно подбора особенностей установки
  prOnReadyScriptAdd('  InitUIDialog(''warenodefilterdialog'',''Место установки'',400); '#13#10);

  // Для найденного товара
  Result:=Result+'<div id=viewsearchingwarediv _oldcodeproduct="-1" _model="-1" _node="-1"></div>'#13#10;    //окно вывода описания товара при поиске
  prOnReadyScriptAdd('  InitUIDialog(''viewsearchingwarediv'',''Описание товара'',800); '#13#10);

  // Для открытия рисунка товара
  Result:=Result+'<div id=viewimagewarediv ><img id=viewimagewareimg></div>'#13#10;    //окно вывода добавляемого рисунка
  prOnReadyScriptAdd('  InitUIDialog(''viewimagewarediv'',''Просмотр выбранного рисунка'',800); '#13#10);
...
begin
  Result:='';
  // завершающие тэги contentdiv(удалено) и maindiv
  Result:=Result+'</div>';
...
begin
 ...
 for i:=1 to j do begin
  ...
  Pict:=Stream.ReadStr; // картинка
  Link:=Stream.ReadStr; // ссылка
 ...
 TableHeader:=TableHeader+'<td title="Видимая">Вид</td>';
 TableHeader:=TableHeader+'<td>Авто</td>';
 TableHeader:=TableHeader+'<td>Moтo</td>';
 TableHeader:=TableHeader+'<td>Начало</td>';
 TableHeader:=TableHeader+'<td>Окончание</td>';
 TableHeader:=TableHeader+'<td>Название</td>';
 TableHeader:=TableHeader+'<td>Ссылка</td>';
 TableHeader:=TableHeader+'<td>Редакт.</td>';
 TableHeader:=TableHeader+'<td>Дата ред.</td>';
 TableHeader:=TableHeader+'<td>Кол-во кликов</td>';
 TableHeader:=TableHeader+'<td><input type=button value=''Добавить'' onclick=''shownewsaedialog();''></td>';
 ...
 for i:=1 to j do begin
   ...
   TableBody:=TableBody+'  <td>'
     ...
     +'href="#" title="Редактировать новость" onclick="ec(''editactionnews'', ''id='+(Code)+''', ''difdict'')"></a>'
     +'<a class="abgslide" style="background-image: url('+DescrImageUrl+'/images/wdell.png); position: static; float: left;" '
     +'href="#" title="Удалить новость" onclick="ec(''delactionnews'', ''id='+(Code)+''', ''difdict'')"></a>'
     +'</td>'#13#10;
...
//---------------------------------------------------------------
// список товаров вебарм
function fnGetWebArmWaresList(var userInf:TEmplInfo; Stream: TBoBMemoryStream): string;
 var
  s, Error: string;
       i, j: integer; // loop local var
  iMaxBrand: integer; // mаx кол. брендов
  iMaxGroup: integer; // max кол. групп в бренде
...
//BlockHeight:=150;
  ...
  s:=s+'<div id=searchresWrap '+fnGetLTWHStyle(_Left+TreeViewWidth+Margins, _Top, LeftBlockWidth-TreeViewWidth-Margins, TreeViewHeight)+'class=divh1><h1>Результаты поиска</h1><div class=inborder id=searchres><table id=searchrestable class=st></table></div></div>';
  if flNewModeCGI then
   ...
   s:=s+'Строка поиска:<input type=text name=templ id=templ style="width: 250px;">'+
  ...
  ' title="В назв. товаров не будут учитываться все символы # ! (пробел) , ; : - . [ ] / + № (  ) \ '' ">&nbsp;Игн.спец.<input type=submit value=''Найти''></form></div>';



  // +++ Блок для отчета по вводимым в поиск данным +++
  ...
  s:=s+' <h1>Скачать инструкцию</h1>'#13#10;
  s:=s+'<div class=inborder id=csrdiv>';
  s:=s+'<a href="#" onclick=''$("#downloadframe")[0].src="'+ScriptName+'/ifbj?act=getlocalfile&filename=product-managers_functional.docx'+'";''>Функционал продукт-менеджера</a>';
  ...
  iMaxBrand:=0;
  s:=s+'<a href="#" id=tab'+IntToStr(iMaxBrand+1)+' class="tablabel" style=''left:' +IntToStr((iMaxBrand)*90)+'px;'' onClick="tabvis(' +IntToStr(fnNextInt(iMaxBrand))+');" title="Оригинальные номера">OE</a>';
  s:=s+'<a href="#" id=tab'+IntToStr(iMaxBrand+1)+' class="tablabel" style=''left:' +IntToStr((iMaxBrand)*90)+'px;'' onClick="tabvis(' +IntToStr(fnNextInt(iMaxBrand))+');" title="Аналоги, заданные в GrossBee">Аналоги(GB)</a>';
  s:=s+'<a href="#" id=tab'+IntToStr(iMaxBrand+1)+' class="tablabel" style=''left:' +IntToStr((iMaxBrand)*90)+'px;'' onClick="tabvis(' +IntToStr(fnNextInt(iMaxBrand))+');" title="Аналоги, найденные через совпадение оригинальных номеров">Аналоги(ОЕ)</a>';
  s:=s+'<a href="#" id=tab'+IntToStr(iMaxBrand+1)+' class="tablabel" style=''left:' +IntToStr((iMaxBrand)*90+1)+'px;'' onClick="tabvis(' +IntToStr(fnNextInt(iMaxBrand))+');" title="Новые, `односторонние` аналоги"> Односторонние</a>';
  s:=s+'<a href="#" id=tab'+IntToStr(iMaxBrand+1)+' class="tablabel" style=''left:' +IntToStr((iMaxBrand)*90)+'px;'' onClick="tabvis(' +IntToStr(fnNextInt(iMaxBrand))+');" title="Кроссы на товары, которых нет в нашем ассортименте"></a>';

  s:=s+'<div class="divh1 tabdiv" id=waredetdiv1 style=''top: 22px; bottom: 5px; left: 5px; right: 5px;''>'#13#10;
  s:=s+'<h1></h1><a class=abgslide id=addornumbyhand title=''Добавить оригинальный номер вручную'' href=''#'' onClick=''addornum(this.name, -1, 1, "", "");'' '+
  ...
  s:=s+'<h1>Правильные оригинальные номера </h1>';
  s:=s+'</div>';
  s:=s+'<div class=divh1 style=''top: 20px; right: 0px; width: 50%; height: 36px;''>'#13#10;
  s:=s+'<h1>Неправильные оригинальные номера </h1>';
  s:=s+'</div>';
  ...
  s:=s+'<h1></h1>'#13#10;
  s:=s+'<a class=abgslide title=''Привязать товар, как односторонний, вручную'' href=''#'' onClick=''addODanalog();'' '+
       'style=''background-image: url(/images/wplus.png); right: 13px; top: 15px; z-index: 10;''></a>'#13#10;
  s:=s+'<div class=divh1 style=''top: 20px; left: 0px; width: 50%; height: 36px;''>'#13#10;
  s:=s+'<h1>Правильные</h1>';
  s:=s+'</div>';
  s:=s+'<div class=divh1 style=''top: 20px; right: 0px; width: 50%; height: 36px;''>'#13#10;
  s:=s+'<h1>Неправильные</h1>';
  s:=s+'</div>';
  ...
  s:=s+'$(''a[id^="ali_"]'').attr(''title'', ''Список товаров выводится по двойному клику на бренде или группе'');'#13#10;

  s:=s+'$(''a[id^="ali_"]'').bind(''click'', function(event) {'#13#10;
  s:=s+'  $(''#biftdid'').attr(''value'', this.id.substr(4));'#13#10;
  s:=s+'  $(''#biftd'').attr(''value'', ''Импорт из TecDoc по ''+((this.previousSibling)?''бренду '':''группе '')+this.innerHTML);'#13#10;
  s:=s+'  $(''#biftd'').css(''visibility'', ''visible'');'#13#10;
  s:=s+'  $(''#bpra'').attr(''value'', ''Отчет по аналогам по ''+((this.previousSibling)?''бренду '':''группе '')+this.innerHTML);'#13#10;
  s:=s+'  $(''#bpra'').attr(''disabled'', false);'#13#10;
  s:=s+'});'#13#10;

// принимаем справочник источников данных
  iMaxBrand := Stream.ReadInt;
...
begin
  ...
  if (ForExport<>'') then begin
    ...
    s:=s+'<h1>Операции получения данных</h1>';
    ...
    s:=s+'Тип данных: <select name=imptype id=exptype>';
    s:=s+ForExport+#13#10;
    s:=s+'</select>'#13#10;
    s:=s+'<div id=changeableinputs><br />'#13#10;
    s:=s+'</div>'#13#10;
    s:=s+'Доп.e-mail:<input type=text id="dop_email" name="dop_email" value="" style="width:450px;"><br>'#13#10;     //
    s:=s+'<input type=button value=''Получить данные'' onclick=''getreport();''>&nbsp;';
    s:=s+'</form><br />'#13#10;
  end;
  ...
  if (ForImport<>'') then begin
    ...
    'if ($(''#ffile'').attr(''value'')=='''') { alert(''Вы не указали файл''); $(''#ffile'')[0].focus();} '+
    'return sendfile(this);">';
    s:=s+'<h1>Операции импорта из файлов</h1>';
    s:=s+'<input type=hidden name=act value=commonimport><input type=hidden name=curUserInfo.UserID value="'+userInf.UserID+'">'+
    'Выберите файл для загрузки: <input type="file" id="ffile" name="ffile" size="50"><br />'+
    'Тип файла импорта: <select name=imptype id=imptype>';
    s:=s+ForImport+#13#10;
    s:=s+'</select>'#13#10;
    s:=s+'<a id=getimporthelp href=#  style="display: none;"><img src="/images/help_2022.png"></a>';
    s:=s+'<br />Доп.e-mail:<input type=text id="dop_emailimp" name="dop_email" value="" style="width:500px;">'#13#10;
    s:=s+'<br /><input type=submit value=''Передать данные''>&nbsp;';
    s:=s+'</form>'#13#10;
  end;

  s:=s+'<h1 id=currentopsh>Текущие операции <span></span> <input type=button value="Обновить" onClick="ec(''showoplist'', '''', ''difdict'');"></h1>';
  s:=s+'<table class=st id=currentopstable><tr><td>Время начала</td><td>Запустил</td><td>Операция</td><td colspan=2>~% выполнения</td></tr></table>';
  s:=s+'<div id=currentopsdiv>В настоящий момент операции импорта/экспорта не выполняются</div>'#13#10;
...
begin
  ...
  MainHeader:=MainHeader+'Начало: <input type=text id=fromdate name=fromdate maxlength=8 size=8 ><img src="/images/calendar.png" style="margin: -3px 2px; cursor: pointer; width: 17px; height: 17px;" onClick="show_calendar(''fromdate'');">';
  MainHeader:=MainHeader+'&nbsp;Окончание: <input type=text id=todate name=todate maxlength=8 size=8 ><img src="/images/calendar.png" style="margin: -3px 2px; cursor: pointer; width: 17px; height: 17px;" onClick="show_calendar(''todate'');">';
  MainHeader:=MainHeader+'&nbsp;<input type=checkbox id=clientauto related=clientmoto checked> Авто&nbsp;<input type=checkbox id=clientmoto related=clientauto checked> Мото';
  MainHeader:=MainHeader+'<br/>Текст: <input id=notiftext maxlength=255 style="width: 600px;">';
  MainHeader:=MainHeader+'<table><tr>';
  MainHeader:=MainHeader+'<td>Тип клиента:</br><select multiple id=clienttype style="height: 500px;">'#13#10;
  MainHeader:=MainHeader+fnListToOptions(Stream);
  MainHeader:=MainHeader+'</select></td>'#13#10;
  MainHeader:=MainHeader+'<td>Категория клиента:</br><select multiple id=clientcategory style="height: 500px;">'#13#10;
  MainHeader:=MainHeader+fnListToOptions(Stream);
  MainHeader:=MainHeader+'</select></td>'#13#10;
  MainHeader:=MainHeader+'<td>Филиал клиента:</br><select multiple id=clientfilial style="height: 500px;">'#13#10;
  MainHeader:=MainHeader+fnListToOptions(Stream);
  MainHeader:=MainHeader+'</select></td>'#13#10;
  ColumnWidth:=300;
  MainHeader:=MainHeader+'<td><select id=individualclientsmethod><option value=0>Дополнительно</option><option value=1>Исключая</option></select><br/>'
...
// ############################################################################
  ...
  TableHeader:=TableHeader+'<td>Начало</td>';
  TableHeader:=TableHeader+'<td>Окончание</td>';
  TableHeader:=TableHeader+'<td>Текст</td>';
  TableHeader:=TableHeader+'<td>Редакт.</td>';
  TableHeader:=TableHeader+'<td>Дата ред.</td>';
  TableHeader:=TableHeader+'<td>Всего к/а</td>';
  TableHeader:=TableHeader+'<td>Ознак. к/а</td>';
  TableHeader:=TableHeader+'<td>Ознак. пользователей</td>';
  TableHeader:=TableHeader+'<td><a class="abgslide" style="position: static; background-image: url(''/images/wplus.png'');" '
                          +'href="#" title="Добавить" onclick="$(''#aenotificationdiv'').attr(''code'', 0).dialog({ title: ''Создание уведомления''}).dialog(''open'');"></a></td>';
  TableHeader:=TableHeader+'</tr>'#13#10;
...
begin
 ...
 s:=s+'  set_sizes();'#13#10;
 s:=s+'  $("#aenotificationdiv").dialog({autoOpen: false, title:"Редактирование уведомления" ,width: $("#aenotificationdiv table").width(), close: clearnotifydlg, buttons: {"Сохранить": savenotification}});'#13#10;
 s:=s+'});'#13#10;
 s:=s+forscript;
 s:=s+'setCookie_("sid", "'+userInf.SessionID+'", getExpDate_(0,0,'+userInf.SessionTimeMinStr+'),"/",0,0);'#13#10;
 s:=s+'</script>';
...
// ############################################################################
  (*TableHeader:=TableHeader+'<tr>';
  TableHeader:=TableHeader+'<td>Код</td>';
  TableHeader:=TableHeader+'<td>Группа</td>';
  TableHeader:=TableHeader+'<td>Наименование</td>';
  TableHeader:=TableHeader+'<td>Значение</td>';
  TableHeader:=TableHeader+'<td>&nbsp;</td>';
  TableHeader:=TableHeader+'<td>Редактировал</td>';
  TableHeader:=TableHeader+'<td>Дата ред.</td>';
  TableHeader:=TableHeader+'</tr>'#13#10;
  TableBody:='';
  *)
 ...
 (* for i:=0 to j do begin
    ...
    if (CanManaged) then begin
      TableBody:=TableBody+'<a class="abgslide" style="background-image: url(/images/user1.png);  width: 19px; height: 19px; right: 0px;" '
                          +'href="#" title="Назначить разрешения" onclick="ec(''showconstroles'', ''id='+Code+''', ''difdict'');"></a>';
    end;

    TableBody:=TableBody+'</div></td>'#13#10;
    TableBody:=TableBody+'  <td style="text-align: left;">'+GetHTMLSafeString(Stream.ReadStr)+'</td>'#13#10; // редактировал
...
begin
  ...
  MainHeader:=MainHeader+'</script>'#13#10;
  TableHeader:=TableHeader+'<td>Код</td>';
  TableHeader:=TableHeader+'<td>Логин</td>';
  TableHeader:=TableHeader+'<td>ФИО</td>';
  TableHeader:=TableHeader+'<td>Подразделение</td>';
  TableHeader:=TableHeader+'<td>Логин GrossBee</td>';
  TableHeader:=TableHeader+'<td><input type=button value="Добавить пользователя" onclick="ec(''aewausers'', ''id=new'','+fnIfStr(flNewModeCGI,'''newbj''','''abj''')+');"></td>';

  TableBody:='<script>';
  for i:=0 to Count-1 do begin
     TableBody:=TableBody+'waul(-1, '+IntToStr(Stream.ReadInt)+', '; // Код
     TableBody:=TableBody+'"'+Stream.ReadStr+'", '; // Логин
     TableBody:=TableBody+''+IntToStr(Stream.ReadInt)+', '; // Код Подразделения
     TableBody:=TableBody+'"'+Stream.ReadStr+'", '; // Логин GrossBee
     TableBody:=TableBody+BoBBoolToStr(Stream.ReadBool); // признак блокированности
...
begin
  s:='';
  s:=s+'<h1>Управление пользователями</h1>'#13#10;
  if flNewModeCGI then
    //s:=s+'<form action="'+ScriptName+'/mppactions" onSubmit="if ($(''#firmtempl'').val()!=''''){return sfbaNew(this);} else{jqswMessage(''Введите строку поиска''); return false;}"><input type=text size=25 id=firmtempl name=firmtempl>'
  ...
  s:=s+'<input type=hidden name=act value=loadfirms><input type=submit value="Найти"></form>'#13#10;
  s:=s+'<div id=firmsdivWrap style=''position: absolute; top: 70px; left: '+IntToStr(0)+'px; bottom: 300px; right: 10px;'''#13#10;
  s:=s+'class=divh1><h1>Контрагенты</h1>'
  ...
  s:=s+'class=divh1><h1>Должностные лица</h1>'
...
begin
  ...
  TableBody:='';
  s:='';
  MainHeader:=MainHeader+'<form onsubmit=''var inp=$("#ordernuminp"); inp.val(mtrim(inp.val())); if (inp.val().length<5) {jqswMessage("Вы не ввели номер заказа");} else {sfbaNew(this)} return false;''>'
                          +'<input type=hidden name=act value="loadorder">'
                          +'Введите номер заказа: <input type=text name=ordernuminp id=ordernuminp><input type=submit value="Искать"></form>';
  ...
  TableHeader:=TableHeader+'<td>Наименование</td>';
  TableHeader:=TableHeader+'<td>Кол-вo</td>';
  TableHeader:=TableHeader+'<td>Ед.изм.</td>';
  TableHeader:=TableHeader+'<td>Цена</td>';
  TableHeader:=TableHeader+'<td>Сумма</td>';
...
begin
  ...
  TableHeader:='';
  TableBody:='';
  MainHeader:=MainHeader+'<h1 style="margin-top:0;">Заявки на регистрацию</h1>';
  ...
  MainHeader:=MainHeader+'class=divh1><h1>Фильтр</h1>'
  ...
  MainHeader:=MainHeader+'Задать период <br />с&nbsp;<input type=text id=fromdate name=fromdate maxlength=8 size=8 value=01.'+FormatDateTime('mm.yy', Date)+'><img src="/images/calendar.png" style="margin: -3px 2px; cursor: pointer; width: 17px; height: 17px;" onClick="show_calendar(''fromdate'');">';
  MainHeader:=MainHeader+'по&nbsp;<input type=text id=todate name=todate maxlength=8 size=8 value='+FormatDateTime('dd.mm.yy', Date)+'><img src="/images/calendar.png" style="margin: -3px 2px; cursor: pointer; width: 17px; height: 17px;" onClick="show_calendar(''todate'');"><br />';
  MainHeader:=MainHeader+'Филиал:&nbsp;<select name=dprt id=dprt '+fnIfStr((fnInIntArray(rolSaleDirector, userInf.Roles)>-1) or (fnInIntArray(rolUiK, userInf.Roles)>-1), '', 'disabled')+'></select>'#13#10;
  MainHeader:=MainHeader+'</td>'#13#10;
  MainHeader:=MainHeader+'<td style=''padding: 5px;''>'#13#10;
  MainHeader:=MainHeader+'<input type=checkbox name=new>необработанные<br />'#13#10;
  MainHeader:=MainHeader+'<input type=checkbox name=processed>принятые<br />'#13#10;
  MainHeader:=MainHeader+'<input type=checkbox name=annulated>отклоненные<br />'#13#10;
  MainHeader:=MainHeader+''#13#10;
  MainHeader:=MainHeader+'</td>'#13#10;
  MainHeader:=MainHeader+'<td>'#13#10;
  MainHeader:=MainHeader+''#13#10;
  MainHeader:=MainHeader+'Название:&nbsp;<br />'#13#10;
  MainHeader:=MainHeader+'<input type=text name=nametempl><br />'#13#10;
  MainHeader:=MainHeader+'<input type=submit value=''Показать заявки''>';
  MainHeader:=MainHeader+''#13#10;
  MainHeader:=MainHeader+'</td>'#13#10;
  ...
  TableHeader:=TableHeader+'<td>Дата заявки</td>';
  TableHeader:=TableHeader+'<td><a title="Статус заявки">Ст.</a></td>';
  TableHeader:=TableHeader+'<td><a title="Действия">Дейст</a></td>';
  TableHeader:=TableHeader+'<td>Филиал</a></td>';
  TableHeader:=TableHeader+'<td>Наименование</td>';
  TableHeader:=TableHeader+'<td>Область</td>';
//        TableHeader:=TableHeader+'<td>ФИО</td>';
//        TableHeader:=TableHeader+'<td>Должность</td>';
  TableHeader:=TableHeader+'<td><a title="Является ли клиентом фирмы">Кл</a></td>';
  TableHeader:=TableHeader+'<td>Адрес</td>';
//        TableHeader:=TableHeader+'<td>Телефон</td>';
   ...
   prOnReadyScriptAdd('      "Выполнить" : function() {'#13#10);
   prOnReadyScriptAdd('        $(''#divtabl1 form'')[0].onsubmit();'#13#10);
   prOnReadyScriptAdd('      },'#13#10);
   prOnReadyScriptAdd('      "Закрыть окно" : function() {'#13#10);
...
begin
  ...
  for i:=0 to Count1 do begin
    ...
    if Stream.ReadInt=aeSuccess then begin
      MainHeader:=MainHeader+'<h1 style="margin:0;">Работа с зонами обслуживания'
      +'<a class="abgslide" style="background-image: url(''/images/wplus.png''); position: static; float:left; display: block;" onclick="aeregzone('''');" href="#" title="Добавить зону обслуживания"></a></h1>';

      TableHeader:=TableHeader+'<td>Зона обслуживания';
      TableHeader:=TableHeader+'</td>';
      TableHeader:=TableHeader+'<td title="E-mail, на который должно уходить письмо о регистрации">E-mail</td>';
      TableHeader:=TableHeader+'<td>Филиал</td>';
      ...
      for i:=0 to Count1-1 do begin
        ...
        TableBody:=TableBody+'<tr id="regzone'+s1+'"><td><div style="position: relative; width: 100%;"><span>'+GetHTMLSafeString(Stream.ReadStr)+'</span>'
                 +'<a class="abgslide" style="background-image: url('+DescrImageUrl+'/images/wedit.png); right: 16px; top: 0px;" '
                                     +'onclick="aeregzone('+s1+');" href="#" title="Редактировать область влияния"></a>'
                 +'<a class="abgslide" style="background-image: url('+DescrImageUrl+'/images/wdell.png); right: 0px; top: 0px;" '
                 +'onclick="ecq(''delregzone'', ''id='+s1+''', ''newbj'', ''Вы действительно хотите удалить зону влияния?'');" href="#" title="Удалить зону влияния"></a>'
      ...
      s:=s+'</script>'#13#10;
       ...
       s:=s+fnWriteSimpleText('Ошибка выполнения: '+GetHTMLSafeString(Error));
    end;
...
begin
  ...
  try
    ...
    s:=s+'</script>;'#13#10;
    s:=s+'<h1>Работа co справочником моделей</h1>'#13#10;
    ...
    s:=s+'class=divh1><h1>Производители</h1>'
        +'<a class="abgslide" style="background-image: url(''/images/wplus.png''); right: 13px; top: 15px; display: block;" onclick="aemanufacturerNew(0, '''', false, '+IntToStr(PageType)+');" href="#" title="Добавить производителя"></a>'
    ...
    s:=s+'class=divh1><h1>Модельные ряды</h1>';
    s:=s+'<a class=abgslide id=addmodelline title=''Добавить модельный ряд'' href=''#'' onClick=''aemodellineNew(0, this.name, '+IntToStr(PageType)+', "", false);'' ';
    ...
    s:=s+'class=divh1><h1>Модели</h1>';
    s:=s+'<div id=modeldiv class=inborder><table id=modeltable class=st></table></div>';
    s:=s+'<a class=abgslide id=addmodel title=''Добавить модель'' href=''#'' onclick=''d'+fnIfStr(PageType=constIsAuto, 'a', 'm')+'mw("");'' ';
    s:=s+'style=''background-image: url(/images/wplus.png); right: 13px; top: 15px; display: none;''></a>';
    s:=s+'</div>'#13#10;
...
begin
  ...
  s:=s+'>';
  s:=s+' <input type=button value="Добавить подузел"';
  ...
  s:=s+'>';
  s:=s+'<br /><br /><input type=button value="Внести изменения" onclick="if (!curnode) {alert(''Не выбран редактируемый узел'')} else {ec(''edittreenode'', ''id=''+curnode.id.substr(6)+''&newname=''+$(''#outername'').val()+''&newsysname=''+$(''#innername'').val()+''&sys='+IntToStr(PageType)+'&vis=''+$(''#nodevisibility'')[0].checked+''&mainnode=''+$(''#mainnodecode'').val(), ''newbj'')}">';
  s:=s+'<br /><br /><input type=button value="Удалить узел" onclick="if (confirm(''Вы действительно хотите удалить узел?'')){if (!curnode) {alert(''Не выбран узел для удаления'')} else {ec(''deltreenode'', ''id=''+curnode.id.substr(6)+''&sys='+IntToStr(PageType)+''', ''newbj'');}}">';
  if (PageType<>cbrMotul) then
    s:=s+' <input type=button value="Отсоединить от главной" onclick="if (!curnode) {alert(''Не выбран узел'')} else {ec(''detachfrommainnode'', ''id=''+curnode.id.substr(6)+''&sys='+IntToStr(PageType)+''', ''newbj'');}">';
  s:=s+#13#10;
  s:=s+'</div>'#13#10;
  Stream.Clear;
...
begin
  ...
  //        s:=s+'<a class=abANew href=# id=critsearchbtn onclick="$(''#critvalue'').autocomplete(''search'' , $(''#critvalue'').val());" style="background-image: url(/images/search_16.png); top: 73px; right: 5px; display: none;"></a><br>';
    ...
    s:=s+'<center>';
  //        s:=s+'<input type=button id=btnSaveCriteria value="Добавить" onclick="add_criteria();">&nbsp;<input type=button value="Закрыть" onclick="$.fancybox().close();">';
    s:=s+'<input type=button id=btnSaveCriteria value="Добавить" onclick="add_criteria();">&nbsp;<input type=button value="Закрыть" onclick="$(''#addcoudiv'').dialog(''close'');">';
    s:=s+'</center>';
    s:=s+'</div>'; // addcoudiv
    s:=s+'</div>'; // скрытый div

    s:=s+'<h1>Не выбран товар для редактирования условий применимости</h1>';
    s:=s+'<input type=button id=btn_addportion value="Добавить блок условий" onclick="ec(''showportion'', ''portion=-1'', ''newbj'');" style="display: none;">';
...
begin
 ...
 // получаю перечень брендов Grossbee
  Stream.Clear;
  ...
  if fnSendReceiveData(csGetBrandsGB, Stream, csWebArm) then begin
    if Stream.ReadInt=aeSuccess then begin
      ...
      for i:=0 to Count-1 do begin
        ...
        // получаю перечень брендов TecDoc
    Stream.Clear;
    Stream.WriteInt(StrToInt(userInf.UserID));
...
// теперь собственно соответствия

    TableHeader:=TableHeader+'<td>Бренд GrossBee</td>';
    TableHeader:=TableHeader+'<td>Бренд TecDoc</td>';
    TableHeader:=TableHeader+'<td><input type=button value="Добавить cooтветствие" onclick="getbrandslink();"></td>';
    TableBody:='<script>';
    ...
    if fnSendReceiveData(csGetLinkBrandsGBTD, Stream, csWebArm) then begin
      if Stream.ReadInt=aeSuccess then begin
         ...
         for i:=0 to Count-1 do begin
           ...
           TableBody:=TableBody+''+IntToStr(Stream.ReadInt)+', '; // Код GrossBee
           TableBody:=TableBody+''+IntToStr(Stream.ReadInt)+' '; // Код TecDoc
           TableBody:=TableBody+');'#13#10; //
         end;
      end else begin
...
begin
  ...
  s:='';
  Count:=Stream.ReadInt;
  TableHeader:=TableHeader+'<td>Бренд</td>';
  TableHeader:=TableHeader+'<td>Сокращение</td>';
  TableHeader:=TableHeader+'<td>Логотип</td>';
  TableHeader:=TableHeader+'<td>Ссылка</td>';
  TableHeader:=TableHeader+'<td>Не показывать в прайсе</td>';
  if flPictNotShow then
    TableHeader:=TableHeader+'<td>Не показывать рисунки TD</td>';
...
begin
   ...
   TitleDop:='<strong>'+fnGetGBDocName(docAccount)+'</strong> № '+DocNum+' от '+DocDate+', <strong>Контракт</strong> № '+ContractNum+'';        //!!!Счет!!!
  ...
  //   Result:=Result+', из <strong>заказа</strong> № <a href="'+ScriptName+'/order?order='+ORDRNum+'">'+ORDRCode+'</a>';
  // end;
  Result:=Result+'</h1>';
  ...
  Result:=Result+'<div id=acccomdiv><div id=deliverydatadivshowdoc>'+DeliveryText+'</div><strong>Комментарий:</strong> <i>'+Commentary+'</i>'#13#10;
  if flMeetPerson then
    Result:=Result+' <span class="meet"><strong >&nbsp; &nbsp;Встречающий:</strong> <i>'+AccMeetText+'</i></span>'#13#10;
  Result:=Result+'</div>';
  ...
  Result:=Result+'arrTableHeaderColName[0]=new Array("Наименование товара","Заказ","Резерв","Ед.изм","Цена","Сумма"); '#13#10;
...
begin
  ...
  LineQty:=Stream.ReadInt;
  TitleDop:=fnGetGBDocName(docInvoice)+' № '+DocNum+' от '+DocDate+', Контракт № '+ContractNum; //!!!Накладная №!!!
  Result:=Result+'<div id=thw><div id=tht></div><img id=thlt src="/images/window/corner-top-left.png"><img id=thrt src="/images/window/corner-top-right.png">'#13#10;
  Result:=Result+'</div>'#13#10; // <div id=thw>
  Result:=Result+'<div id=tcdbackground></div><div id=mfb></div><img id=mflb src="/images/window/corner-bottom-left.png"><img id=mfrb src="/images/window/corner-bottom-right.png">';
  Result:=Result+'<div id=mainheaderwrap>';
  ...
  Result:=Result+'<h1 id=ordernumh1 class=doctitle>'+TitleDop;
  if flMeetPerson then
    Result:=Result+'<br>Встречающий: '+AccMeetText+'</h1><br clear=both>'
  else
  ...
  Result:=Result+'<script>'#13#10;
  Result:= Result+'var arrTableHeaderColName=[];'#13#10;
  Result:=Result+'arrTableHeaderColName[0]=new Array("Наименование товара","К-во","Ед.изм","Цена","Сумма"); '#13#10;
  Result:=Result+'TStream.arlen='+IntToStr(LineQty)+';  '#13#10;
  Result:=Result+' TStream.artable= new Array(); '#13#10;
  for i:=0 to LineQty-1 do begin
    WareTitle:=Stream.ReadStr;
...
// страница управление сайта Motul
...
begin
  ...
  if (KindOfPage='3') then begin
    if (NeedFullPage)  then begin
      ...
      Result:=Result+'  <ul>'#13#10;
      Result:=Result+'    <li><a href="#motul-action-div" onclick="ec(''motulsitetab'',''kindofpage=3'',''newbj'');" title="Акции">Акции</a></li>'#13#10;
      Result:=Result+'    <li><a href="#motul-wares-div" onclick="if (document.getElementById(''tablecontent-wares'').rows.length==0){ ec(''motulsitetab'',''kindofpage=4'',''newbj'');}" title="Продукты">Продукты</a></li>'#13#10;
      Result:=Result+'    <li><a href="#motul-info-div" onclick="if (document.getElementById(''tablecontent-info'').rows.length==0){ ec(''motulsitetab'',''kindofpage=5'',''newbj'');}" title="Итоги/Информация покупателю">Итоги/Информация покупателю</a></li>'#13#10;
      Result:=Result+'  </ul>'#13#10;
  ...
  if (KindOfPage='4') then begin
    ...
    for i := 0 to Count-1 do begin
      ...
      if fsize=0 then begin
        Result:=Result+'ImageSize:0, '; // размер картинки
        Result:=Result+'Image:"", '
      end
      else begin
        Result:=Result+'ImageSize:'+IntToStr(fsize)+', '; // размер картинки
        try
          temp:='';
      ...
      result:=fnWebArmAutenticateNew(curUserInfo,fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'psw'),fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'lgn'),sid,curUserInfo.strOther.Values['ip'],curUserInfo.strOther.Values['agent'],ThreadData);
        ...
        if result=aeSuccess then begin
          ...
          if (curUserInfo.strOther.Values['act']='changepass') then begin
            ...
            nmProc := 'prNewsPage'; // имя процедуры/функции
            StreamNew.Clear;
            ...
            nmProc := 'prShowNotificationOrd'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prShowActionNews'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prAEDNotification'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            if (Length(s)<8) or not TryEncodeDate(2000+StrToIntDef(Copy(s, 7, 2), 0), StrToIntDef(Copy(s, 4, 2), 0), StrToIntDef(Copy(s, 1, 2), 0), Date) then begin
             setErrorCommandStr(Stream,'Неправильная дата - "'+s+'"');
            end
            else begin
            ...
            nmProc := 'prAEActionNews'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            s:=fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'todate');
            if (Length(s)<8) or not TryEncodeDate(2000+StrToIntDef(Copy(s, 7, 2), 0), StrToIntDef(Copy(s, 4, 2), 0), StrToIntDef(Copy(s, 1, 2), 0), Date) then begin
             setErrorCommandStr(Stream,'Неправильная дата Окончания- "'+s+'"');
            end
            else begin
              StreamNew.WriteDouble(Date);
              s:=fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'fromdate');
              if (Length(s)<8) or not TryEncodeDate(2000+StrToIntDef(Copy(s, 7, 2), 0), StrToIntDef(Copy(s, 4, 2), 0), StrToIntDef(Copy(s, 1, 2), 0), Date) then begin
               setErrorCommandStr(Stream,'Неправильная дата Начала - "'+s+'"');
              end
              else begin
                StreamNew.WriteDouble(Date);
            ...
            nmProc := 'fnShowNotificationWA'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToIntDef(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'code'), 0));
            ...
            nmProc := 'prEditSysOption'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prSaveSysOption'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToIntDef(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id'), 0));
            ...
            nmProc := 'prAEWebArmUser'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prSaveWebArmUsers'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToIntDef(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id'), -1));
            ...
            nmProc := 'prBlockWebArmUser'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToIntDef(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id'),0));
            ...
            nmProc := 'prProductWareSearch'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteByte(1); // проверять на соответствие менеджеру
            StreamNew.WriteByte(0); // признак передачи самого маленького набора - Id товара, бренда, группы, наименования товара
            if (trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id'))<>'') then begin
              StreamNew.WriteByte(0); // признак запроса товара по коду группы
              StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id')));
            end else begin
              StreamNew.WriteByte(1); // признак запроса товара по коду группы
            ...
            nmProc := 'prGetManufacturerList'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(isWe);
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            j:=StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'sys'));
            StreamNew.WriteInt(j); // 0 - все производители
            if (j=21) then begin
               StreamNew.WriteBool(false);    // можно показывать невидимые,  так как там завязка на ОН
            end else begin
               StreamNew.WriteBool(true);    // в остальных случаях - не показывать невидимые
            end;
            ...
            nmProc := 'prWebArmGetRegionalFirms'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prLoadOrder'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prWebArmShowAccount'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'id')));
            ...
            nmProc := 'prWebArmEditAccountHeader'; // имя процедуры/функции
            s:='';
            ...
            nmProc := 'prCheckContracts'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prGetCompareWaresInfo'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToInt(curUserInfo.FirmID));
            StreamNew.WriteInt(StrToIntDef(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'contract'), 0));
            ...
            nmProc := 'rWebarmContractList'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToIntDef(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'firmid'), 0));
            ...
            nmProc := 'prWebArmGetFilteredAccountList'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ShortFilter:=fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'shortfilter')='true';
            if (ShortFilter) then begin
               StreamNew.WriteDouble(System.SysUtils.Date-14); // за две недели
            end else begin
               ...
               if temp='' then begin
                 ...
                 if (Length(temp)<8) or not TryEncodeDate(2000+StrToIntDef(Copy(temp, 7, 2), 0), StrToIntDef(Copy(temp, 4, 2), 0), StrToIntDef(Copy(temp, 1, 2), 0), Date) then begin
                   raise EBOBError.Create('Неправильная дата - '+temp);
                 end;
                StreamNew.WriteDouble(Date);
                DateBegin:=Date;
               end;
            ...
            if ((fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'filterexecuted')<>'') and ((DateEnd-DateBegin)<>0) and (i=-1)) then begin
               raise EBOBError.Create('Запрос исполненных счетов выдаст слишком много результатов. Пожалуйста сократите интервал дат до одного дня или укажите конкретного контрагента.');
            end;
            ...
            nmProc := 'prWebArmGetTransInvoicesList'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prWebArmMakeSecondAccount'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'id')));
            ...
            nmProc := 'prWebArmEditAccountLine'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'accid')));
            ...
            nmProc := 'prCommonGetNodeWares'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.FirmID));
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prGetFilterValues'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.FirmID));
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToIntDef(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'node'), -1));
            ...
            nmProc := 'prGetWareInfoView'; // имя процедуры/функции
            StreamNew.Clear;
           ...
           (*else if (curUserInfo.strOther.Values['act']='FindByVIN') then begin
               ...
               if (length(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'vin'))=17) and (not wizardsearch) then begin
                 ...
                 if fnSendReceiveData(i, StreamNew, csServerManage) then begin
                    if StreamNew.ReadInt=aeSuccess then begin
                      ...
                      s1:=StreamNew.ReadStr;
                      s1:=' По введенному данным результаты не найдены.'#13#10+fnIfStr((System.Pos('URL',s1)>0),copy(s1,1,System.Pos('URL',s1)-1),s1);
                      setErrorCommandStr(Stream,'jqswMessageError("Ошибка : '+GetJSSafeString(s)+'");');
                    end;
                 end
                 else begin
                   setErrorCommand(StreamNew,Stream);
                 ...
                 if (length(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'vin'))<>17) then begin
                   setErrorCommandStr(Stream,'jqswMessageError("Ошибка : '+GetJSSafeString('VIN-код должен состоять из 17 символов!')+'");');
                 end;
                 if (wizardsearch) then begin
                   setErrorCommandStr(Stream,'jqswMessageError("alert("Ошибка: '+GetJSSafeString('В указанном значении '+s1+' присутствуют запрещенные символы!')+'");');
                 end;
             ...
             nmProc := 'getorignumandanalogs'; // имя процедуры/функции
             ...
             nmProc := 'prWebArmGetFirmUsers'; // имя процедуры/функции
             StreamNew.Clear;
             StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
             StreamNew.WriteInt(StrToIntDef(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet,'id'),0));
             ...
             StreamNew.WriteBool(false);    // показывать невидимые, так как тут вроде управление
             prGetManufacturerList(StreamNew,ThreadData);
             if StreamNew.ReadInt=aeSuccess then begin
               Stream.Clear;
               Stream.WriteInt(aeSuccess);
             ...
             nmProc := 'prProductAddOrigNum'; // имя процедуры/функции
             StreamNew.Clear;
             StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
             StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'ornumware')));     // куда присоединять
             StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'manufacturerid')));// код производителя авто
             StreamNew.WriteStr(trim(UTF8ToANSI(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'ornumtext'))));             // собственно оригинальный номер
             ...
             nmProc := 'prProductDelOrigNum'; // имя процедуры/функции
             StreamNew.Clear;
             StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
             StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'wareid')));     // откуда отцеплять
             StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'numid')));      // что отцеплять
             ...
             StreamNew.Clear;
             StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
             StreamNew.WriteByte(0); // проверять на соответствие менеджеру 0=нет
             StreamNew.WriteByte(1); // признак передачи среднего набора - Id товара, бренд, группа, наименования товара
             StreamNew.WriteByte(1); // признак запроса товара по шаблону поиска
             StreamNew.WriteByte(byte(UpperCase(trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'ignorespec')))='ON')); //
             StreamNew.WriteStr(trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'templ')));
             prProductWareSearch(StreamNew,ThreadData);
             if StreamNew.ReadInt=aeSuccess then begin
               ...
               for i := 1 to j do begin
                 s:=s+'addwaretosearchlist1(' + IntToStr(StreamNew.ReadInt)+ {код товара}
                 ', "' + (StreamNew.ReadStr)+'"'+ {бренд}
                 ', "' + (StreamNew.ReadStr)+'"'+ {группа}
                 ', "'+ StringReplace(Stream.ReadStr, '"', '`',[rfReplaceAll, rfIgnoreCase]) +'", -1, -1, false);'#13#10; {наименование товара}
               end;

               s:=s+'$(''a[id^="srware_"]'').bind(''click'', function(event) {'#13#10;
               s:=s+'  tabvis(tabnum_);'#13#10;
               s:=s+'  this.title="Показать допданные товара";'#13#10;
               s:=s+'});'#13#10;
             ...
             StreamNew.Clear;
             StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
             StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'wareid')));     // товар
             StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'numid')));      // ОЕ
             StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'wrong')));      // на какой признак меняем
             StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'srcid')));      // код источника
             prProductWareSearch(StreamNew,ThreadData);
             if StreamNew.ReadInt=aeSuccess then begin
               Stream.Clear;
               Stream.WriteInt(aeSuccess);
             ...
             nmProc := 'prGetModelLineList'; // имя процедуры/функции
             ...
             StreamNew.WriteBool(false);    // показывать невидимые
            ...
            nmProc := 'prManufacturerAdd'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            s:=trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'sys'));
            ...
            nmProc := 'prModelLineAdd'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            s:=trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'sys'));
            StreamNew.WriteInt(StrToInt(s)); //
            ...
            nmProc := 'prModelLineEdit'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prManufacturerEdit'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            s:=trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id'));
            StreamNew.WriteInt(StrToInt(s)); //
            ...
            nmProc := 'prModelLineDel'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            s:=trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id'));
            StreamNew.WriteInt(StrToInt(s)); //
            ...
            nmProc := 'prManufacturerDel'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            s:=trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'sys'));
            StreamNew.WriteInt(StrToInt(s)); //
            ...
            nmProc := 'prTNANodeEdit'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            s:=trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'sys'));
            StreamNew.WriteInt(StrToInt(s)); //
            StreamNew.WriteInt(StrToInt(trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id'))));
            StreamNew.WriteStr('');                                               // имя ноды
            StreamNew.WriteStr('');                                               // системное имя ноды
            StreamNew.WriteInt(fnIfInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'vis')='true', 1, 0));  // видимость
            StreamNew.WriteInt(-1);                                                // главная нода
            ...
            nmProc := 'prWebArmSetFirmMainUser'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            prWebArmSetFirmMainUser(StreamNew,ThreadData);
             if StreamNew.ReadInt=aeSuccess then begin
               ...
               if (fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, '_login')<>'') then begin
                 s:=s+'$.fancybox.close();'#13#10;
                 s:=s+'jqswMessage("Учетная запись для нового главного пользователя успешно создана. Логин и пароль отправлены на email пользователя.");'#13#10;
               end;
               Stream.WriteStr(s);
             end else begin
               setErrorCommand(StreamNew,Stream);
            ...
            if not (Sys in constAllSys) then begin
              setErrorCommandStr(StreamNew,'Неизвестный код системы - '+fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id'));
            end
            else begin
              nmProc := 'prGetModelLineModels'; // имя процедуры/функции
              StreamNew.Clear;
              StreamNew.WriteInt(StrToInt(curUserInfo.FirmID));
              StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
              StreamNew.WriteInt(StrToInt(trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id'))));
              StreamNew.WriteBool(true); // топы вверх
              StreamNew.WriteBool(true); // не показывать невидимые
            ...
            nmProc := 'prTNANodeDel'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            s:=trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'sys'));
            StreamNew.WriteInt(StrToInt(s)); //
            ...
            StreamNew.WriteInt(fnIfInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'vis')='true', 1, 0));  // видимость
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'mainnode')));         // главная нода
            prTNANodeEdit(StreamNew,ThreadData);
             if StreamNew.ReadInt=aeSuccess then begin
               s1:=StreamNew.ReadStr;
               Stream.Clear;
            ...
            nmProc := 'prTNANodeAdd'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToInt(trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'sys'))));
            StreamNew.WriteInt(StrToInt(trim(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id'))));
            ...
            nmProc := 'prShowPortion'; // имя процедуры/функции
            ...
            nmProc := 'prGetCateroryValues'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prMarkPortions'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'model')));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'node')));
            ...
            nmProc := 'prCommonGetRestsOfWares'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToInt(curUserInfo.FirmID));
            ...
            nmProc := 'prShowConditionPortions'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'model')));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'node')));
            ...
            nmProc := 'prSavePortion'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'model')));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'node')));
           ...
           if StreamNew.ReadInt=aeSuccess then begin
            ...
            nmProc := 'prAddLinkBrandsGBTD'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
           ...
           if StreamNew.ReadInt=aeSuccess then begin
            ...
            nmProc := 'prWebArmDeleteRegionalZone'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id')));
           ...
           if StreamNew.ReadInt=aeSuccess then begin
            ...
            nmProc := 'prWebArmInsertRegionalZone'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteStr(UTF8ToAnsi(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, '_name')));
            StreamNew.WriteStr(UTF8ToAnsi(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, '_email')));
            ...
            nmProc := 'annregord'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id')));
            ...
            if StreamNew.ReadInt=aeSuccess then begin
              ...
              Stream.Clear;
              Stream.WriteInt(aeSuccess);
              s:=s+'$("#regordertr'+fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id')+'")[0].cells[1].innerHTML="<a class=ahint title=''Аннулирована''>2</a>";'#13#10;
            ...
            nmProc := 'prWebArmGetOrdersToRegister'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            if (Length(s1)<8) or not TryEncodeDate(2000+StrToIntDef(Copy(s1, 7, 2), 0), StrToIntDef(Copy(s1, 4, 2), 0), StrToIntDef(Copy(s1, 1, 2), 0), DateBegin) then begin
              setErrorCommandStr(Stream,'Неправильная дата - '+s1);
            end else begin
            ...
            nmProc := 'prGetFilialList'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prDelLinkBrandsGBTD'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'GBCode')));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'TDCode')));
           ...
           if StreamNew.ReadInt=aeSuccess then begin
            ...
            nmProc := 'getfirmlist'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
           ...
           if StreamNew.ReadInt=aeSuccess then begin
            ...
            nmProc := 'prWebArmGetFirmAccountList'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'id')));
            ...
            nmProc := 'prProductPage'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prCOUPage'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prManageLogotypesPage'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prManageBrands'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
          ...
          {else if (curUserInfo.strOther.Values['act']='accountsreestr') then begin
            ...
            if (fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'type')='99') then   begin
              nmProc := 'prShowGBAccountOrd'; // имя процедуры/функции
              StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
              StreamNew.WriteInt(isWe);
              StreamNew.WriteStr(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'code'));
            ...
            if (fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'type')='102') then   begin
              nmProc := 'prShowGBOutInvoice'; // имя процедуры/функции
              StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
              StreamNew.WriteInt(isWe);
              StreamNew.WriteStr(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'code'));
              StreamNew.WriteInt(StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'emplid')));
            ...
            StreamNew.Clear;
            nmProc := 'prMotulSitePage'; // имя процедуры/функции
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            StreamNew.Clear;
            nmProc := 'prMotulSiteManage'; // имя процедуры/функции
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            if (StrToInt(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'kindofoperation'))=mspAddAct) then  begin
               ...
               try
                 ...
                 setErrorCommandStr(StreamNew,'Некорректная дата начала акции');
               end;
               try
                 DateEnd:=StrToDate(fnGetFieldStrList(curUserInfo.strPost,curUserInfo.strGet, 'action-todate'));
               except
                 setErrorCommandStr(StreamNew,'Некорректная дата окончания акции');
               end;
               StreamNew.WriteDouble(DateBegin);
               StreamNew.WriteDouble(DateEnd);
            ...
            StreamNew.Clear;
            nmProc := 'prLogotypeEdit'; // имя процедуры/функции
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prImportPage'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prShowSysOptionsPage'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'prAccountsReestrPage'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            prAccountsReestrPage(StreamNew,ThreadData);
            if StreamNew.ReadInt=aeSuccess then begin
            ...
            nmProc := 'prShowWebArmUsers'; // имя процедуры/функции
            StreamNew.Clear;
            StreamNew.WriteInt(StrToInt(curUserInfo.UserID));
            ...
            nmProc := 'mpp'; // имя процедуры/функции
            Stream.Clear;
            OnReadyScript:='';
            globToHeader:='';
            globToHeader:=globToHeader+'<link rel="stylesheet" type="text/css" href="/product.css'+(*floatTostr(random)+*)'">'#13#10;
            ...
            nmProc := 'order'; // имя процедуры/функции
            Stream.Clear;
            OnReadyScript:='';
            globToHeader:='';
            globToHeader:=globToHeader+'<link rel="stylesheet" type="text/css" href="/product.css'+(*floatTostr(random)+*)'">'#13#10;
            ...
            nmProc := 'prTNAManagePage'; // имя процедуры/функции
            Stream.Clear;
            OnReadyScript:='';
            globToHeader:='';
            globToHeader:=globToHeader+'<link rel="stylesheet" type="text/css" href="/product.css'+(*floatTostr(random)+*)'">'#13#10;
            ...
            nmProc := 'mppregord'; // имя процедуры/функции
            Stream.Clear;
            OnReadyScript:='';
            globToHeader:='';
            globToHeader:=globToHeader+'<link rel="stylesheet" type="text/css" href="/product.css'+(*floatTostr(random)+*)'">'#13#10;
