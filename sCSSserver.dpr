...
begin
  ...
  if IsServiceCSS then begin  // сервис
    prMessageLOG('Запуск сервиса '+Application.Name, 'system');
    try
      Installing:= FindCmdLineSwitch('INSTALL',['-','\','/'], True) or
                FindCmdLineSwitch('UNINSTALL',['-','\','/'], True);
      SvcMgr.Application.Initialize;
//prMessageLOGS('Пройдена инициализация', 'startlog', False);
      SvcMgr.Application.CreateForm(TServiceCSS, ServiceCSS);
  //  prMessageLOGS('Создана форма', 'startlog', False);
  SvcMgr.Application.Run;
    except
      on E: Exception do prMessageLOG('Ошибка работы сервиса '+E.Message, 'system');
    end;
    prMessageLOG('Остановка сервиса '+Application.Name, 'system');
  end else begin             // приложение
    prMessageLOG('Старт программы '+Application.Name, 'system');
    try
      Forms.Application.Initialize;
      Forms.Application.CreateForm(TForm1, Form1);
      Form1.Show;
      Forms.Application.ProcessMessages;
      if fnServerInit then begin // выполняет подключения к ГроссБии, ADS и пр.
        Forms.Application.Run;
      end;
      if (Form1<>nil) then Form1.Close;
    except
      on E: Exception do prMessageLOG('Ошибка работы программы '+E.Message, 'system');
    end;
    prMessageLOG('Завершение работы программы '+Application.Name, 'system');
  end;
  prMessageLOG(StringOfChar('+', 50), 'system');
end.
